...
pub async fn seed_map_render_tile(
	...
	let pixels = tauri::async_runtime::spawn_blocking(move || seed_map::render_tile(request))
		.await
...
pub async fn seed_map_find_features(
	...
	Ok(tauri::async_runtime::spawn_blocking(move || seed_map::find_features(query))
		.await
		.map_err(std::io::Error::other)??)
}
...
pub async fn seed_map_spawn(
	...
	Ok(tauri::async_runtime::spawn_blocking(move || seed_map::spawn(seed, edition, version))
		.await
		.map_err(std::io::Error::other)??)
}
...
pub async fn seed_map_scan_ores(
	...
	Ok(
		tauri::async_runtime::spawn_blocking(move || seed_map::ores::scan_ores(request))
			.await
			.map_err(std::io::Error::other)??,
	)
}
...
#[tauri::command]
pub async fn seed_map_read_level_dat(path: String) -> Result<seed_map::LevelDatInfo> {
	Ok(tauri::async_runtime::spawn_blocking(move || seed_map::read_level_dat(path))
		.await
		.map_err(std::io::Error::other)??)
}
