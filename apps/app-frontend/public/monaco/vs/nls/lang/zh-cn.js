...
globalThis._VSCODE_NLS_MESSAGES = [
	...
	'使用旧差异算法。',
	'使用高级差异算法。',
	'使用外部 `@vscode/diff` 包提供的高级差异算法(纯 JavaScript)。',
	'使用外部 `@vscode/diff` 包提供的高级差异算法(WebAssembly)。',
	'控制差异编辑器是否显示未更改的区域。',
	'控制用于未更改区域的行数。',
	'控制将多少行用作未更改区域的最小值。',
	...
	'悬停已启用。',
	'悬停已禁用。',
	'按住 `{0}` 或 `Alt` (`#editor.multiCursorModifier#` 的相反修饰键)时显示悬停',
	'控制是否显示悬停提示。',
	'控制显示悬停提示前的等待时间 (毫秒)。',
	...
	'用于降低悬停详细程度的图标。',
	'正在加载...',
	'由于性能原因，长线的呈现已暂停。可通过 `editor.stopRenderingLineAfter` 配置此设置。[不再显示](command:{0})',
	'由于性能原因，在进行标记化时跳过了长行。解析长度阈值可通过 `editor.maxTokenizationLineLength` 进行配置。[不再显示](command:{0})',
	'提高悬停详细程度',
	'降低悬停详细程度',
	'查看问题',
	'没有可用的快速修复',
	...
	'无法注册“{0}”。此属性已注册。',
	'无法注册“{0}”。设置不得同时声明 "policy" 和 "policyReference"。',
	...
	'对比度最高的前景色，适用于需要在各种背景下实现最大可读性的文本或图标。仅在未被组件覆盖时使用此颜色。',
