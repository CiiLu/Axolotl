...
globalThis._VSCODE_NLS_MESSAGES = [
	...
	'使用舊版差異演算法。',
	'使用進階版差異演算法。',
	'使用來自外部 `@vscode/diff` 套件 (單純 JavaScript) 的進階 Diff 演算法。',
	'使用來自外部 `@vscode/diff` 套件 (WebAssembly) 的進階 Diff 演算法。',
	'控制差異編輯器是否顯示未變更的區域。',
	'控制未變更區域的使用行數。',
	'控制未變更區域的最小使用行數。',
	...
	'暫留已啟用。',
	'暫留已停用。',
	"按住 '{0}' 或 'Alt' (與 '#editor.multiCursorModifier#' 相反的修飾元) 時，會顯示懸停",
	'控制是否顯示暫留。',
	'控制暫留顯示的延遲時間 (以毫秒為單位)。',
	'控制當滑鼠移過時，是否應保持顯示暫留。',
	'控制暫留隱藏的延遲時間 (以毫秒為單位)。需要啟用 `#editor.hover.sticky#`。',
	'如果有空間，則偏好在行上方顯示游標。',
	'控制是否顯示長行警告暫留，例如跳過權杖化或暫停轉譯時。',
	...
	'當顯示內嵌完成時，會停用快速建議',
	'已針對所有權杖類型啟用快速建議',
	'所有權杖類型的快速建議會顯示為準刪除文字',
	'已停用所有權杖類型的快速建議',
	'當顯示內嵌完成時，會停用所有權杖類型的快速建議',
	'允許在字串內顯示即時建議。',
	'允許在註解中顯示即時建議。',
	'允許在字串與註解以外之處顯示即時建議。',
	...
	'降低暫留詳細程度的圖示。',
	'正在載入...',
	'由於效能原因，已暫停過長的單行轉譯。這可透過 `editor.stopRenderingLineAfter` 進行設定。[不要再顯示](command:{0})',
	'因效能的緣故，已跳過將長的行 Token 化。您可透過 `editor.maxTokenizationLineLength` 設定。[不要再顯示](command:{0})',
	'提高暫留詳細程度',
	'降低暫留詳細程度',
	'檢視問題',
	'沒有可用的快速修正',
	...
	'行 {0}，欄 {1}: {2}',
	'先開啟文字編輯器，前往某一行或某個位移。',
	'移至行。輸入行號，選擇性地後面接著冒號和欄號。',
	'切換以零為基礎的位移',
	'前往行 {0}，欄 {1}。按 ENTER 進行瀏覽。',
	'先開啟文字編輯器，前往某一行或某個位移。',
	'輸入要前往的字元位置 (從 0 到 {0})。',
	'輸入要前往的字元位置 (從 1 到 {0})。',
	...
	"無法註冊 '{0}'。此屬性已經註冊。",
	"無法註冊 '{0}'。設定不能同時宣告 'policy' 和 'policyReference'。",
	"無法註冊 '{0}'。已向 {2} 註冊關聯的原則 {1}。若要將另一個設定連結至相同的原則，請使用 'policyReference'。",
	'傳回有關內容索引鍵資訊的命令',
	'空的內容索引鍵運算式',
	...
	'最高對比度的前景色，適用於需要在各種背景中達到最大可讀性的文字或圖示。此色彩僅在未被元件覆寫時使用。',
