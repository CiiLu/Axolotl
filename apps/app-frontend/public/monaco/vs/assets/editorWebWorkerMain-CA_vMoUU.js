...
` +
	...
	const Jn = new pi()
	function Rt(t) {
		Yn(t) || Jn.onUnexpectedError(t)
	}
	function bi(t) {
		Yn(t) || Jn.onUnexpectedExternalError(t)
	}
	function Kt(t) {
	...
	const en = 'Canceled'
	function Yn(t) {
		return t instanceof Zn ? !0 : t instanceof Error && t.name === en && t.message === en
	}
	...
	function wi(t, e = 'Unreachable') {
		throw new Error(e)
	}
	function xi(t, e = 'unexpected state') {
		if (!t) throw typeof e == 'string' ? new Y(`Assertion Failed: ${e}`) : e
	}
	function ht(t) {
		if (!t()) {
			debugger
			;(t(), Rt(new Y('Assertion Failed')))
		}
	}
	function tn(t, e) {
		let n = 0
	...
	;(function (t) {
		...
		t.from = o
		function* l(x) {
			for (let y = x.length - 1; y >= 0; y--) yield x[y]
		}
		t.reverse = l
		function u(x) {
		...
		function m(x, y) {
			let S = 0
			for (const V of x) if (y(V, S++)) return !0
			return !1
		}
		t.some = m
		function h(x, y) {
			let S = 0
			for (const V of x) if (!y(V, S++)) return !1
			return !0
		}
		t.every = h
		function f(x, y) {
			for (const S of x) if (y(S)) return S
		}
		t.find = f
		function* p(x, y) {
			for (const S of x) y(S) && (yield S)
		}
		t.filter = p
		function* g(x, y) {
			let S = 0
			for (const V of x) yield y(V, S++)
		}
		t.map = g
		function* d(x, y) {
			let S = 0
		...
		t.flatMap = d
		function* w(...x) {
			for (const y of x) Li(y) ? yield* y : yield y
		}
		t.concat = w
		function _(x, y, S) {
			let V = S
			for (const U of x) V = y(V, U)
			return V
		}
		t.reduce = _
		function v(x) {
		...
		t.length = v
		function* L(x, y, S = x.length) {
			for (
				y < -x.length && (y = 0),
					y < 0 && (y += x.length),
					S < 0 ? (S += x.length) : S > x.length && (S = x.length);
				y < S;
				y++
			)
				yield x[y]
		...
		function k(x, y = Number.POSITIVE_INFINITY) {
			const S = []
			if (y === 0) return [S, x]
			const V = x[Symbol.iterator]()
			for (let U = 0; U < y; U++) {
				const B = V.next()
				if (B.done) return [S, t.empty()]
				S.push(B.value)
			}
			return [
				S,
		...
		async function D(x) {
			const y = []
			for await (const S of x) y.push(S)
			return y
		}
		t.asyncToArray = D
		async function W(x) {
			let y = []
			for await (const S of x) y = y.concat(S)
			return y
		}
	...
	const Xt = class Xt {
		...
		add(e) {
			if (!e || e === Ie.None) return e
			if (e === this) throw new Error('Cannot register a disposable on itself!')
			return (
				this._isDisposed
					? Xt.DISABLE_DISPOSED_WARNING ||
						console.warn(
							new Error(
	...
	class Ni {
		...
		clear() {
			...
			for (; e !== X.Undefined; ) {
				const n = e.next
				;((e.prev = X.Undefined), (e.next = X.Undefined), (e = n))
			}
			;((this._first = X.Undefined), (this._last = X.Undefined), (this._size = 0))
		}
		unshift(e) {
		...
		_insert(e, n) {
			const r = new X(e)
			if (this._first === X.Undefined) ((this._first = r), (this._last = r))
			else if (n) {
				const i = this._last
	...
	function tr(t, e) {
		...
		return (
			...
			Ri && (n = '［' + n.replace(/[aouei]/g, '$&$&') + '］'),
			n
		)
	}
	function F(t, e, ...n) {
		return tr(typeof t == 'number' ? Ci(t, e) : e, n)
	}
	function Ci(t, e) {
		const n = Si()?.[t]
		if (typeof n != 'string') {
	...
	const ve = globalThis
	let ie
	typeof ve.vscode < 'u' && typeof ve.vscode.process < 'u'
		? (ie = ve.vscode.process)
		: typeof process < 'u' && typeof process?.versions?.node == 'string' && (ie = process)
	const Ei = typeof ie?.versions?.electron == 'string' && ie?.type === 'renderer'
	if (typeof ie == 'object') {
		;((nn = ie.platform === 'win32'),
			(rn = ie.platform === 'darwin'),
			(sn = ie.platform === 'linux'),
			sn && ie.env.SNAP && ie.env.SNAP_REVISION,
			ie.env.CI || ie.env.BUILD_ARTIFACTSTAGINGDIRECTORY || ie.env.GITHUB_WORKSPACE,
			(At = Oe),
			(on = Oe))
		const t = ie.env.VSCODE_NLS_CONFIG
		if (t)
			try {
				const e = JSON.parse(t)
				;((At = e.userLocale),
					(rr = e.osLocale),
					(on = e.resolvedLanguage || Oe),
					(Ai = e.languagePack?.translationsConfigFile))
			} catch {}
		nr = !0
	} else
		typeof navigator == 'object' && !Ei
			? ((_e = navigator.userAgent),
				(nn = _e.indexOf('Windows') >= 0),
				(rn = _e.indexOf('Macintosh') >= 0),
				(_e.indexOf('Macintosh') >= 0 || _e.indexOf('iPad') >= 0 || _e.indexOf('iPhone') >= 0) &&
					navigator.maxTouchPoints &&
					navigator.maxTouchPoints > 0,
				(sn = _e.indexOf('Linux') >= 0),
				_e?.indexOf('Mobi') >= 0,
				(an = !0),
				(on = er() || Oe),
				(At = navigator.language.toLowerCase()),
				(rr = At))
	...
	const dt = nn,
		...
		Mi = nr,
		Pi = an,
		Ti = an && typeof ve.importScripts == 'function' ? ve.origin : void 0,
		xe = _e,
		Fi = typeof ve.postMessage == 'function' && !ve.importScripts
	;(() => {
		if (Fi) {
			const t = []
			...
			return (n) => {
				const r = ++e
				;(t.push({ id: r, callback: n }), ve.postMessage({ vscodeScheduleAsyncWork: r }, '*'))
			}
		}
		return (t) => setTimeout(t)
	})()
	const Di = !!(xe && xe.indexOf('Chrome') >= 0)
	;(xe && xe.indexOf('Firefox') >= 0,
		!Di && xe && xe.indexOf('Safari') >= 0,
		xe && xe.indexOf('Edg/') >= 0,
		xe && xe.indexOf('Android') >= 0)
	let je
	const ln = globalThis.vscode
	if (typeof ln < 'u' && typeof ln.process < 'u') {
		const t = ln.process
		je = {
			get platform() {
				return t.platform
		...
		typeof process < 'u' && typeof process?.versions?.node == 'string'
			...
			: (je = {
					get platform() {
						return dt ? 'win32' : ki ? 'darwin' : 'linux'
					},
					get arch() {},
					get env() {
						return {}
	...
	const Et = je.cwd,
		sr = je.env,
		Ii = je.platform,
		Vi = globalThis.performance.now.bind(globalThis.performance)
	class kt {
		static create(e) {
			return new kt(e)
		}
		constructor(e) {
			;((this._now = e === !1 ? Date.now : Vi),
	...
	const Bi = 100,
		ir = 6e4
	function ar() {
		return !!sr.VSCODE_DEV
	}
	var un
	;(function (t) {
		t.None = () => Ie.None
		function e(C, N, A) {
			return f(C, () => {}, 0, void 0, N ?? !0, void 0, A)
		}
		t.defer = e
		function n(C) {
			return (N, A = null, M) => {
				...
				return (
					(I = C(
						(H) => {
							if (!E) return (I ? I.dispose() : (E = !0), N.call(A, H))
						},
						null,
						M,
					)),
		...
		t.onceIf = r
		function s(C, N, A) {
			return m((M, E = null, I) => C((H) => M.call(E, N(H)), null, I), A)
		}
		t.map = s
		function i(C, N, A) {
			return m(
		...
		t.forEach = i
		function o(C, N, A) {
			return m((M, E = null, I) => C((H) => N(H) && M.call(E, H), null, I), A)
		}
		t.filter = o
		function l(C) {
			return C
		...
		function u(...C) {
			return (N, A = null, M) => {
				const E = _i(...C.map((I) => I((H) => N.call(A, H))))
				return h(E, M)
			}
		}
		t.any = u
		...
		function f(C, N, A = 100, M = !1, E = !1, I, H) {
			let K,
				G,
				se,
				ee = 0,
			...
			const He = {
					...
					onWillAddFirstListener() {
						K = C((g1) => {
							;(ee++,
								(G = N(G, g1)),
								M && !se && (ct.fire(G), (G = void 0)),
								(oe = () => {
									const p1 = G
									;((G = void 0), (se = void 0), (!M || ee > 1) && ct.fire(p1), (ee = 0))
								}),
								typeof A == 'number'
									? (se && clearTimeout(se), (se = setTimeout(oe, A)))
									: se === void 0 && ((se = null), queueMicrotask(oe)))
						})
					},
				...
				ct = new ge(He)
			return (H?.add(ct), ct.event)
		}
		t.debounce = f
		function p(C, N = 0, A, M) {
			return t.debounce(C, (E, I) => (E ? (E.push(I), E) : [I]), N, void 0, A ?? !0, void 0, M)
		}
		t.accumulate = p
		function g(C, N, A = 100, M = !0, E = !0, I, H) {
			let K,
				G,
				se,
				ee = 0
					...
					onWillAddFirstListener() {
						K = C((ct) => {
							;(ee++,
								(G = N(G, ct)),
								se === void 0 &&
									(M && (He.fire(G), (G = void 0), (ee = 0)),
									typeof A == 'number'
										? (se = setTimeout(() => {
												;(E && ee > 0 && He.fire(G), (G = void 0), (se = void 0), (ee = 0))
											}, A))
										: ((se = 0),
											queueMicrotask(() => {
												;(E && ee > 0 && He.fire(G), (G = void 0), (se = void 0), (ee = 0))
		...
		t.latch = d
		function w(C, N, A) {
			return [t.filter(C, N, A), t.filter(C, (M) => !N(M), A)]
		}
		t.split = w
		function _(C, N, A = !1, M = [], E) {
			let I = M.slice(),
				H
			ar() &&
				((H = {
					...
					timerId: setTimeout(() => {
						I &&
							...
							((H.warned = !0),
							console.warn(
								`[Event.buffer][${N}] potential LEAK detected: ${I.length} events buffered for ${ir / 1e3}s without being consumed. Buffered here:`,
							),
							H.stack.print())
					}, ir),
					warned: !1,
				}),
				E && E.add(mt(() => clearTimeout(H.timerId))))
			const K = () => {
				...
				I
					? (I.push(oe),
						ar() &&
							...
							((H.warned = !0),
							console.warn(
								`[Event.buffer][${N}] potential LEAK detected: ${I.length} events buffered without being consumed. Buffered here:`,
							),
							H.stack.print()))
					: ee.fire(oe)
			})
			E && E.add(G)
			const se = () => {
					;(I?.forEach((oe) => ee.fire(oe)), (I = null), K())
				},
				ee = new ge({
					onWillAddFirstListener() {
						G || ((G = C((oe) => ee.fire(oe))), E && E.add(G))
					},
					onDidAddFirstListener() {
						I && (A ? setTimeout(se) : se())
					},
					onDidRemoveLastListener() {
		...
		class k {
			...
			filter(N) {
				return (this.steps.push((A) => (N(A) ? A : L)), this)
			}
			reduce(N, A) {
				let M = A
			...
			latch(N = (A, M) => A === M) {
				...
				return (
					this.steps.push((E) => {
						const I = A || !N(E, M)
						return ((A = !1), (M = E), I ? E : L)
					}),
					this
				)
			}
			evaluate(N) {
				for (const A of this.steps) if (((N = A(N)), N === L)) break
				return N
			}
		}
		function D(C, N, A = (M) => M) {
			const M = (...K) => H.fire(A(...K)),
				E = () => C.on(N, M),
				I = () => C.removeListener(N, M),
				H = new ge({ onWillAddFirstListener: E, onDidRemoveLastListener: I })
			return H.event
		}
		t.fromNodeEventEmitter = D
		function W(C, N, A = (M) => M) {
			const M = (...K) => H.fire(A(...K)),
				E = () => C.addEventListener(N, M),
				I = () => C.removeEventListener(N, M),
				H = new ge({ onWillAddFirstListener: E, onDidRemoveLastListener: I })
			return H.event
		}
		...
		function x(C, N) {
			...
			return ((E.cancel = A), N && E.finally(() => or(M, N)), E)
		}
		t.toPromise = x
		function y(C, N) {
			return C((A) => N.fire(A))
		...
		class V {
			constructor(N, A) {
				...
				;((this.emitter = new ge(M)), A && A.add(this.emitter))
			}
			beginUpdate(N) {
				this._counter++
			}
		...
		function U(C, N) {
			return new V(C, N).emitter.event
		}
		t.fromObservable = U
		function B(C) {
			return (N, A, M) => {
				...
				const H = {
					...
					endUpdate() {
						;(E--, E === 0 && (C.reportChanges(), I && ((I = !1), N.call(A))))
					},
				...
				;(C.addObserver(H), C.reportChanges())
				const K = {
					dispose() {
						C.removeObserver(H)
					},
	...
	let cn = at,
		qi = -1
	const Qt = class Qt {
		constructor(e, n, r = (Qt._idPool++).toString(16).padStart(3, '0')) {
		...
		check(e, n) {
			...
			if (
				...
				const [i, o] = this.getMostFrequentStack(),
					...
					u = `[${this.name}] potential listener LEAK detected, having ${n} listeners already. MOST frequent listener (${o}):`
				;(console.warn(u), console.warn(i))
				const c = o / n > 0.3 ? 'dominated' : 'popular',
					m = new Pt(c, u, i, n, l)
...
`),
	...
	class Pt extends Error {
		constructor(e, n, r, s, i) {
			;(super(
				i
					? `[${i}] potential listener LEAK detected, ${e}`
	...
	class ge {
		constructor(e) {
			;((this._size = 0),
				...
				(this._leakageMon = this._options?.leakWarningThreshold
					? new hn(
							e?.onListenerError ?? Rt,
							this._options?.leakWarningThreshold ?? qi,
			...
			return (
				(this._event ??= (e, n, r) => {
					if (this._leakageMon && this._size > this._leakageMon.threshold ** 2) {
						...
						return ((this._options?.onListenerError || Rt)(m), Ie.None)
					}
					if (this._disposed) return Ie.None
					n && (e = e.bind(n))
					const s = new mn(e)
					let i
					;(this._leakageMon &&
						this._size >= Math.ceil(this._leakageMon.threshold * 0.2) &&
						((s.stack = Mt.create()), (i = this._leakageMon.check(s.stack, this._size + 1))),
						this._listeners
							? this._listeners instanceof mn
								? ((this._deliveryQueue ??= new Wi()), (this._listeners = [this._listeners, s]))
		...
		_deliver(e, n) {
			if (!e) return
			const r = this._options?.onListenerError || Rt
			if (!r) {
				e.value(n)
				return
			}
		...
		fire(e) {
			if (
				...
				if (this._listeners instanceof mn) this._deliver(this._listeners, e)
				else {
	...
	class zi {
		constructor(e, n) {
			;((this.lastCache = void 0),
				...
				typeof e == 'function'
					? ((this._fn = e), (this._computeKey = Hi))
	...
	class dn {
		constructor(e) {
			;((this.executor = e), (this._state = Ve.Uninitialized))
		}
		get value() {
			if (this._state === Ve.Uninitialized) {
				this._state = Ve.Running
				try {
					this._value = this.executor()
				} catch (e) {
					this._error = e
				} finally {
					this._state = Ve.Completed
				}
			} else if (this._state === Ve.Running)
	...
	const Le = class Le {
		static getInstance(e) {
			return Le.cache.get(Array.from(e).join(','))
		}
		static getLocales() {
			return Le._locales.value
		}
		constructor(e) {
	...
	;((Le.ambiguousCharacterData = new dn(() =>
		JSON.parse(
		...
		(Le.cache = new zi((e) => {
			...
			function r(h) {
				const f = new Map()
				for (let p = 0; p < h.length; p += 2) f.set(h[p], h[p + 1])
				return f
			}
			function s(h, f) {
				const p = new Map(h)
				for (const [g, d] of f) p.set(g, d)
				return p
			}
			function i(h, f) {
				if (!h) return f
				const p = new Map()
				for (const [g, d] of h) f.has(g) && p.set(g, d)
				return p
			}
			const o = Le.ambiguousCharacterData.value
		...
		(Le._locales = new dn(() =>
			Object.keys(Le.ambiguousCharacterData.value).filter((e) => !e.startsWith('_')),
		)))
	let gt = Le
	const ot = class ot {
		static getRawData() {
		...
		static getData() {
			return (
				this._data || (this._data = new Set([...Object.values(ot.getRawData())].flat())),
				this._data
			)
		}
	...
	class aa {
		...
		async sendMessage(e, n, r) {
			...
			return new Promise((i, o) => {
				;((this._pendingReplies[s] = { resolve: i, reject: o }),
					this._send(new na(this._workerId, s, e, n, r)))
			})
		}
		listen(e, n, r) {
			let s = null
			const i = new ge({
				onWillAddFirstListener: () => {
					;((s = String(++this._lastSentReq)),
						...
						this._send(new ra(this._workerId, s, e, n, r)))
				},
				onDidRemoveLastListener: () => {
					;(this._pendingEmitters.delete(s), this._send(new ia(this._workerId, s)), (s = null))
				},
			})
			return i.event
		}
			...
			const r = {
				get: (s, i) => (
					typeof i == 'string' &&
						!s[i] &&
						(hr(i)
							? (s[i] = (o) => this.listen(e, i, o))
							: cr(i)
								? (s[i] = this.listen(e, i, void 0))
								: i.charCodeAt(0) === 36 &&
									(s[i] = async (...o) => (await n?.(), this.sendMessage(e, i, o)))),
					s[i]
				),
			}
		...
		_handleRequestMessage(e) {
			...
			this._handler.handleMessage(e.channel, e.method, e.args).then(
				(s) => {
					this._send(new ur(this._workerId, n, s, void 0))
				},
				(s) => {
					;(s.detail instanceof Error && (s.detail = Kt(s.detail)),
						this._send(new ur(this._workerId, n, void 0, Kt(s))))
				},
			)
		}
		_handleSubscribeEventMessage(e) {
			const n = e.req,
				r = this._handler.handleEvent(
					...
					this._send(new sa(this._workerId, n, s))
				})
	...
	function cr(t) {
		return t[0] === 'o' && t[1] === 'n' && lr(t.charCodeAt(2))
	}
	function hr(t) {
		return /^onDynamic/.test(t) && lr(t.charCodeAt(9))
	}
	class oa {
		constructor(e, n) {
			;((this._localChannels = new Map()),
				(this._remoteChannels = new Map()),
				(this._protocol = new aa({
					sendMessage: (r, s) => {
						e(r, s)
					},
		...
		_handleMessage(e, n, r) {
			if (e === wn && n === ta) return this.initialize(r[0])
			const s = e === wn ? this.requestHandler : this._localChannels.get(e)
	...
	let xn = !1
	function la(t) {
		if (xn) throw new Error('WebWorker already initialized!')
		xn = !0
		const e = new oa(
	...
	new dn(() => new Uint8Array(256))
	function mr(t, e) {
		return ((e << 5) - e + t) | 0
	}
	function ca(t, e) {
		e = mr(149417, e)
		for (let n = 0, r = t.length; n < r; n++) e = mr(t.charCodeAt(n), e)
		return e
	}
	class fr {
		constructor(e) {
	...
	function ha(t, e, n) {
		return new Me(new fr(t), new fr(e)).ComputeDiff(n).changes
	}
	class Ge {
		static Assert(e, n) {
	...
	class Me {
		constructor(e, n, r = null) {
			...
			const [s, i, o] = Me._getElements(e),
				[l, u, c] = Me._getElements(n)
		...
		static _getElements(e) {
			...
			if (Me._isStringArray(n)) {
				const r = new Int32Array(n.length)
				for (let s = 0, i = n.length; s < i; s++) r[s] = ca(n[s], 0)
				return [n, r, !0]
			}
		...
		ElementsAreStrictEqual(e, n) {
			...
			const r = Me._getStrictElement(this._originalSequence, e),
				s = Me._getStrictElement(this._modifiedSequence, n)
			return r === s
		}
		...
		_ComputeDiff(e, n, r, s, i) {
			...
			return (i && (l = this.PrettifyChanges(l)), { quitEarly: o[0], changes: l })
		}
		ComputeDiffRecursive(e, n, r, s, i) {
			...
			if (e > n || r > s) {
				...
				return (
					r <= s
						? (Ge.Assert(e === n + 1, 'originalStart should only be one more than originalEnd'),
							(h = [new ke(e, 0, r, s - r + 1)]))
						: e <= n
							? (Ge.Assert(r === s + 1, 'modifiedStart should only be one more than modifiedEnd'),
								(h = [new ke(e, n - e + 1, r, 0)]))
							: (Ge.Assert(e === n + 1, 'originalStart should only be one more than originalEnd'),
								Ge.Assert(r === s + 1, 'modifiedStart should only be one more than modifiedEnd'),
								(h = [])),
					h
			...
			const o = [0],
				l = [0],
				u = this.ComputeRecursionPoint(e, n, r, s, o, l, i),
				c = o[0],
				m = l[0]
			if (u !== null) return u
			if (!i[0]) {
				...
				return (
					i[0]
						? (f = [new ke(c + 1, n - (c + 1) + 1, m + 1, s - (m + 1) + 1)])
						: (f = this.ComputeDiffRecursive(c + 1, n, m + 1, s, i)),
					this.ConcatenateChanges(h, f)
				)
			}
			return [new ke(e, n - e + 1, r, s - r + 1)]
		}
		WALKTRACE(e, n, r, s, i, o, l, u, c, m, h, f, p, g, d, w, _, v) {
			let L = null,
				k = null,
				D = new dr(),
				W = n,
			...
			do {
				...
				;(U === W || (U < x && c[U - 1] < c[U + 1])
					? ((h = c[U + 1]),
						(g = h - y - s),
						h < S && D.MarkNextChange(),
						(S = h),
						D.AddModifiedElement(h + 1, g),
						(y = U + 1 - e))
					: ((h = c[U - 1] + 1),
						(g = h - y - s),
						h < S && D.MarkNextChange(),
						(S = h - 1),
						D.AddOriginalElement(h, g + 1),
						(y = U - 1 - e)),
					V >= 0 && ((c = this.m_forwardHistory[V]), (e = c[0]), (W = 1), (x = c.length - 1)))
			} while (--V >= -1)
			if (((L = D.getReverseChanges()), v[0])) {
				let U = p[0] + 1,
					B = w[0] + 1
				if (L !== null && L.length > 0) {
					const C = L[L.length - 1]
					;((U = Math.max(U, C.getOriginalEnd())), (B = Math.max(B, C.getModifiedEnd())))
				}
				k = [new ke(U, f - U + 1, B, d - B + 1)]
			} else {
				;((D = new dr()),
					(W = o),
					(x = l),
					(y = p[0] - w[0] - u),
					(S = 1073741824),
					(V = _ ? this.m_reverseHistory.length - 1 : this.m_reverseHistory.length - 2))
				do {
					const U = y + i
					;(U === W || (U < x && m[U - 1] >= m[U + 1])
						? ((h = m[U + 1] - 1),
							...
							h > S && D.MarkNextChange(),
							(S = h + 1),
							D.AddOriginalElement(h + 1, g + 1),
							(y = U + 1 - i))
						: ((h = m[U - 1]),
							(g = h - y - u),
							h > S && D.MarkNextChange(),
							(S = h),
							D.AddModifiedElement(h + 1, g + 1),
							(y = U - 1 - i)),
						V >= 0 && ((m = this.m_reverseHistory[V]), (i = m[0]), (W = 1), (x = m.length - 1)))
				} while (--V >= -1)
				k = D.getChanges()
			}
			return this.ConcatenateChanges(L, k)
		}
		ComputeRecursionPoint(e, n, r, s, i, o, l) {
			...
			for (let y = 1; y <= g / 2 + 1; y++) {
				let S = 0,
					V = 0
				;((m = this.ClipDiagonalBound(v - y, y, v, d)),
					(h = this.ClipDiagonalBound(v + y, y, v, d)))
				for (let B = m; B <= h; B += 2) {
					;(B === m || (B < h && w[B - 1] < w[B + 1]) ? (u = w[B + 1]) : (u = w[B - 1] + 1),
						(c = u - (B - v) - k))
					const C = u
					...
					if (
						((w[B] = u),
						u + c > S + V && ((S = u), (V = c)),
						!x && Math.abs(B - L) <= y - 1 && u >= _[B])
					)
						return (
							(i[0] = u),
							(o[0] = c),
							C <= _[B] && y <= 1448
								? this.WALKTRACE(v, m, h, k, L, f, p, D, w, _, u, n, i, c, s, o, x, l)
								: null
						)
				}
				const U = (S - e + (V - r) - y) / 2
				if (this.ContinueProcessingPredicate !== null && !this.ContinueProcessingPredicate(S, U))
					return (
						(l[0] = !0),
						(i[0] = S),
						(o[0] = V),
						U > 0 && y <= 1448
							? this.WALKTRACE(v, m, h, k, L, f, p, D, w, _, u, n, i, c, s, o, x, l)
							: (e++, r++, [new ke(e, n - e + 1, r, s - r + 1)])
					)
				;((f = this.ClipDiagonalBound(L - y, y, L, d)),
					(p = this.ClipDiagonalBound(L + y, y, L, d)))
				for (let B = f; B <= p; B += 2) {
					;(B === f || (B < p && _[B - 1] >= _[B + 1]) ? (u = _[B + 1] - 1) : (u = _[B - 1]),
						(c = u - (B - L) - D))
					const C = u
					for (; u > e && c > r && this.ElementsAreEqual(u, c); ) (u--, c--)
					if (((_[B] = u), x && Math.abs(B - v) <= y && u <= w[B]))
						return (
							(i[0] = u),
							(o[0] = c),
							C >= w[B] && y <= 1448
				...
				if (y <= 1447) {
					let B = new Int32Array(h - m + 2)
					;((B[0] = v - m + 1),
						Xe.Copy2(w, m, B, 1, h - m + 1),
						this.m_forwardHistory.push(B),
						(B = new Int32Array(p - f + 2)),
						(B[0] = L - f + 1),
						Xe.Copy2(_, f, B, 1, p - f + 1),
						this.m_reverseHistory.push(B))
				}
			}
			return this.WALKTRACE(v, m, h, k, L, f, p, D, w, _, u, n, i, c, s, o, x, l)
		}
		PrettifyChanges(e) {
			...
			for (let n = e.length - 1; n >= 0; n--) {
				...
				for (let h = 1; ; h++) {
					const f = r.originalStart - h,
						p = r.modifiedStart - h
					if (
						f < s ||
						p < i ||
						(o && !this.OriginalElementsAreEqual(f, f + r.originalLength)) ||
						(l && !this.ModifiedElementsAreEqual(p, p + r.modifiedLength))
					)
						break
					const d =
						(f === s && p === i ? 5 : 0) +
						this._boundaryScore(f, r.originalLength, p, r.modifiedLength)
					d > c && ((c = d), (u = h))
				}
			...
			if (this._hasStrings)
				for (let n = 1, r = e.length; n < r; n++) {
					const s = e[n - 1],
						...
						f = h - m
					if (o < 5 && c < 20 && f < 20) {
						const p = this._findBetterContiguousSequence(l, c, m, f, o)
						if (p) {
							const [g, d] = p
		...
		_findBetterContiguousSequence(e, n, r, s, i) {
			...
			for (let h = e; h < o; h++)
				for (let f = r; f < l; f++) {
					const p = this._contiguousSequenceScore(h, f, i)
					p > 0 && p > u && ((u = p), (c = h), (m = f))
				}
			return u > 0 ? [c, m] : null
		}
		...
		ConcatenateChanges(e, n) {
			...
			if (this.ChangesOverlap(e[e.length - 1], n[0], r)) {
				...
				return (
					Xe.Copy(e, 0, s, 0, e.length - 1),
					(s[e.length - 1] = r[0]),
					Xe.Copy(n, 1, s, e.length, n.length - 1),
					s
				)
			} else {
				const s = new Array(e.length + n.length)
				return (Xe.Copy(e, 0, s, 0, e.length), Xe.Copy(n, 0, s, e.length, n.length), s)
			}
		}
	...
	class q {
		...
		with(e = this.lineNumber, n = this.column) {
			return e === this.lineNumber && n === this.column ? this : new q(e, n)
		}
		delta(e = 0, n = 0) {
		...
		isBeforeOrEqual(e) {
			return q.isBeforeOrEqual(this, e)
		}
		...
		static lift(e) {
			return new q(e.lineNumber, e.column)
		}
	...
	class R {
		...
		static plusRange(e, n) {
			...
			return (
				...
				n.endLineNumber > e.endLineNumber
					...
					: n.endLineNumber === e.endLineNumber
						? ((i = n.endLineNumber), (o = Math.max(n.endColumn, e.endColumn)))
		...
		static intersectRanges(e, n) {
			...
			const l = n.startLineNumber,
				...
				m = n.endColumn
			return (
				r < l ? ((r = l), (s = u)) : r === l && (s = Math.max(s, u)),
				i > c ? ((i = c), (o = m)) : i === c && (o = Math.min(o, m)),
				r > i || (r === i && s > o) ? null : new R(r, s, i, o)
			)
		}
		equalsRange(e) {
	...
	class yn {
		constructor(e) {
			const n = gr(e)
			;((this._defaultValue = n), (this._asciiMap = yn._createAsciiMap(n)), (this._map = new Map()))
		}
	...
	function ga() {
		if (bt === null) {
			bt = new yn(0)
			const t = ` 	<>'"、。｡､，．：；‘〈「『〔（［｛｢｣｝］）〕』」〉’｀～…|`
			for (let n = 0; n < t.length; n++) bt.set(t.charCodeAt(n), 1)
			const e = '.,;:'
			for (let n = 0; n < e.length; n++) bt.set(e.charCodeAt(n), 2)
		}
		return bt
	}
	class Tt {
		static _createLink(e, n, r, s, i) {
			...
			return {
				range: { startLineNumber: r, startColumn: s + 1, endLineNumber: r, endColumn: o + 2 },
				url: n.substring(s, o + 1),
			}
		}
		static computeLinks(e, n = da()) {
			const r = ga(),
				s = []
				...
				for (; c < u; ) {
					...
					if (f === 13) {
						...
						switch (v) {
							...
							case 34:
							case 96:
								h === v ? (L = 1) : h === 39 || h === 34 || h === 96 ? (L = 0) : (L = 1)
								break
							case 42:
								L = h === 42 ? 1 : 0
								break
						...
						L === 1 && (s.push(Tt._createLink(r, l, i, m, c)), (_ = !0))
					} else if (f === 12) {
						let L
						;(v === 91 ? ((g = !0), (L = 0)) : (L = r.get(v)), L === 1 ? (_ = !0) : (f = 13))
					} else ((f = n.nextState(f, v)), f === 0 && (_ = !0))
					;(_ && ((f = 1), (p = !1), (g = !1), (w = !1), (m = c + 1), (h = v)), c++)
				}
				f === 13 && s.push(Tt._createLink(r, l, i, m, u))
			}
			return s
		}
	}
	...
	const Jt = class Jt {
		...
		valueSetsReplace(e, n, r) {
			let s = null
			for (let i = 0, o = e.length; s === null && i < o; i++) s = this.valueSetReplace(e[i], n, r)
			return s
		}
	...
	Jt.INSTANCE = new Jt()
	let _n = Jt
	const pr = Object.freeze(function (t, e) {
	...
	;(function (t) {
		function e(n) {
			return n === t.None || n === t.Cancelled || n instanceof Dt
				? !0
				: !n || typeof n != 'object'
					? !1
		...
		;((t.isCancellationToken = e),
			(t.None = Object.freeze({ isCancellationRequested: !1, onCancellationRequested: un.None })),
			(t.Cancelled = Object.freeze({ isCancellationRequested: !0, onCancellationRequested: pr })))
	})(Ft || (Ft = {}))
	class Dt {
		constructor() {
		...
		get onCancellationRequested() {
			return this._isCancelled
				? pr
				: (this._emitter || (this._emitter = new ge()), this._emitter.event)
		}
		dispose() {
	...
	class ba {
		...
		get token() {
			return (this._token || (this._token = new Dt()), this._token)
		}
		cancel() {
			this._token ? this._token instanceof Dt && this._token.cancel() : (this._token = Ft.Cancelled)
		}
		dispose(e = !1) {
			;(e && this.cancel(),
				this._parentListener?.dispose(),
				this._token ? this._token instanceof Dt && this._token.dispose() : (this._token = Ft.None))
		}
	}
	class vn {
		constructor() {
	...
	;(function () {
		const e = [
			...
			r = []
		for (const s of e) {
			const [i, o, l, u, c, m, h, f, p] = s
			if ((r[o] || ((r[o] = !0), (xa[l] = o), (ya[l.toLowerCase()] = o)), !n[u])) {
				if (((n[u] = !0), !c))
					throw new Error(`String representation missing for key code ${u} around scan code ${l}`)
				;(It.define(u, c), Nn.define(u, f || c), Sn.define(u, p || f || c))
			}
			m && (wa[m] = u)
		}
	})()
	...
	;(function (t) {
		...
		function s(l) {
			return Sn.keyCodeToStr(l)
		}
		t.toUserSettingsGeneral = s
		function i(l) {
			return Nn.strToKeyCode(l) || Sn.strToKeyCode(l)
		}
	...
	function Ca(t, e) {
		if (t === null || typeof t != 'object') throw new wr(e, 'Object', t)
	}
	function J(t, e) {
		if (typeof t != 'string') throw new wr(e, 'string', t)
	}
	const Se = Ii === 'win32'
	function $(t) {
		return t === Z || t === ae
				...
				if (!(o === c - 1 || l === 1))
					if (l === 2) {
						if (
							s.length < 2 ||
							i !== 2 ||
							s.charCodeAt(s.length - 1) !== Be ||
							s.charCodeAt(s.length - 2) !== Be
						) {
							if (s.length > 2) {
								const m = s.lastIndexOf(n)
								;(m === -1
									? ((s = ''), (i = 0))
									: ((s = s.slice(0, m)), (i = s.length - 1 - s.lastIndexOf(n))),
									(o = c),
									(l = 0))
								continue
							} else if (s.length !== 0) {
						...
						(s.length > 0 ? (s += `${n}${t.slice(o + 1, c)}`) : (s = t.slice(o + 1, c)),
							(i = c - o - 1))
				;((o = c), (l = 0))
			} else u === Be && l !== -1 ? ++l : (l = -1)
		}
	...
	const te = {
			resolve(...t) {
				...
				for (let s = t.length - 1; s >= -1; s--) {
					let i
					if (s >= 0) {
						if (((i = t[s]), J(i, `paths[${s}]`), i.length === 0)) continue
					} else
						e.length === 0
							? (i = Et())
							: ((i = sr[`=${e}`] || Et()),
								(i === void 0 ||
									(i.slice(0, 2).toLowerCase() !== e.toLowerCase() && i.charCodeAt(2) === ae)) &&
									(i = `${e}\\`))
					const o = i.length
					let l = 0,
						u = '',
					...
					if (o === 1) $(m) && ((l = 1), (c = !0))
						...
						if (((c = !0), $(i.charCodeAt(1)))) {
							let h = 2,
								f = h
							for (; h < o && !$(i.charCodeAt(h)); ) h++
							if (h < o && h !== f) {
								const p = i.slice(f, h)
								for (f = h; h < o && $(i.charCodeAt(h)); ) h++
								if (h < o && h !== f) {
									for (f = h; h < o && !$(i.charCodeAt(h)); ) h++
									;(h === o || h !== f) && ((u = `\\\\${p}\\${i.slice(f, h)}`), (l = h))
								}
							}
						} else l = 1
					else
						Re(m) &&
							i.charCodeAt(1) === Ne &&
							((u = i.slice(0, 2)), (l = 2), o > 2 && $(i.charCodeAt(2)) && ((c = !0), (l = 3)))
					if (u.length > 0)
						if (e.length > 0) {
					...
					if (r) {
						if (e.length > 0) break
					} else if (((n = `${i.slice(l)}\\${n}`), (r = c), c && e.length > 0)) break
				}
				return ((n = Vt(n, !r, '\\', $)), r ? `${e}\\${n}` : `${e}${n}` || '.')
			},
			normalize(t) {
				J(t, 'path')
				const e = t.length
				...
				if ($(i))
					...
					Re(i) &&
						t.charCodeAt(1) === Ne &&
						((r = t.slice(0, 2)), (n = 2), e > 2 && $(t.charCodeAt(2)) && ((s = !0), (n = 3)))
				let o = n < e ? Vt(t.slice(n), !s, '\\', $) : ''
				if (
					(o.length === 0 && !s && (o = '.'),
					o.length > 0 && $(t.charCodeAt(e - 1)) && (o += '\\'),
					!s && r === void 0 && t.includes(':'))
				) {
					if (o.length >= 2 && Re(o.charCodeAt(0)) && o.charCodeAt(1) === Ne) return `.\\${o}`
					let l = t.indexOf(':')
					do if (l === e - 1 || $(t.charCodeAt(l + 1))) return `.\\${o}`
			...
			isAbsolute(t) {
				...
				return $(n) || (e > 2 && Re(n) && t.charCodeAt(1) === Ne && $(t.charCodeAt(2)))
			},
			join(...t) {
			...
			relative(t, e) {
				if ((J(t, 'from'), J(e, 'to'), t === e)) return ''
				const n = te.resolve(t),
					r = te.resolve(e)
						...
						: L === v
							? _ > v
								...
								: w > v
									? '..\\'.repeat(w - 1 - L) + '..'
									: ''
							: '..\\'.repeat(w - L) + d.slice(L).join('\\')
				}
				let s = 0
				for (; s < t.length && t.charCodeAt(s) === ae; ) s++
				let i = t.length
				for (; i - 1 > s && t.charCodeAt(i - 1) === ae; ) i--
				const o = i - s
				let l = 0
				for (; l < e.length && e.charCodeAt(l) === ae; ) l++
				let u = e.length
				for (; u - 1 > l && e.charCodeAt(u - 1) === ae; ) u--
				const c = u - l,
					m = o < c ? o : c
				let h = -1,
					f = 0
				...
				if (f !== m) {
					...
					if (c > m) {
						if (e.charCodeAt(l + f) === ae) return r.slice(l + f + 1)
						if (f === 2) return r.slice(l + f)
					}
					;(o > m && (t.charCodeAt(s + f) === ae ? (h = f) : f === 2 && (h = 3)),
						h === -1 && (h = 0))
				}
				let p = ''
				for (f = s + h + 1; f <= i; ++f)
					(f === i || t.charCodeAt(f) === ae) && (p += p.length === 0 ? '..' : '\\..')
				return (
					(l += h),
					p.length > 0 ? `${p}${r.slice(l, u)}` : (r.charCodeAt(l) === ae && ++l, r.slice(l, u))
				)
			},
			toNamespacedPath(t) {
				if (typeof t != 'string' || t.length === 0) return t
				const e = te.resolve(t)
				if (e.length <= 2) return t
				if (e.charCodeAt(0) === ae) {
					if (e.charCodeAt(1) === ae) {
						const n = e.charCodeAt(2)
						if (n !== Ra && n !== Be) return `\\\\?\\UNC\\${e.slice(2)}`
					}
				} else if (Re(e.charCodeAt(0)) && e.charCodeAt(1) === Ne && e.charCodeAt(2) === ae)
					return `\\\\?\\${e}`
				return e
			},
			dirname(t) {
				...
				if ($(s)) {
					...
					Re(s) && t.charCodeAt(1) === Ne && ((n = e > 2 && $(t.charCodeAt(2)) ? 3 : 2), (r = n))
				let i = -1,
					o = !0
				for (let l = e - 1; l >= r; --l)
					if ($(t.charCodeAt(l))) {
			...
			basename(t, e) {
				...
				let n = 0,
					...
					i
				if (
					(t.length >= 2 && Re(t.charCodeAt(0)) && t.charCodeAt(1) === Ne && (n = 2),
					...
					for (i = t.length - 1; i >= n; --i) {
						...
						if ($(u)) {
							...
							(l === -1 && ((s = !1), (l = i + 1)),
								o >= 0 && (u === e.charCodeAt(o) ? --o === -1 && (r = i) : ((o = -1), (r = l))))
					}
			...
			extname(t) {
				...
				let e = 0,
					...
					i = !0,
					o = 0
				t.length >= 2 && t.charCodeAt(1) === Ne && Re(t.charCodeAt(0)) && (e = r = 2)
				for (let l = t.length - 1; l >= e; --l) {
					...
					;(s === -1 && ((i = !1), (s = l + 1)),
						u === Be ? (n === -1 ? (n = l) : o !== 1 && (o = 1)) : n !== -1 && (o = -1))
				}
				return n === -1 || s === -1 || o === 0 || (o === 1 && n === s - 1 && n === r + 1)
					? ''
					: t.slice(n, s)
			},
			format: xr.bind(null, '\\'),
			parse(t) {
				...
				if ($(s)) {
					if (((r = 1), $(t.charCodeAt(1)))) {
						let h = 2,
							f = h
						for (; h < n && !$(t.charCodeAt(h)); ) h++
						if (h < n && h !== f) {
							for (f = h; h < n && $(t.charCodeAt(h)); ) h++
							if (h < n && h !== f) {
								for (f = h; h < n && !$(t.charCodeAt(h)); ) h++
								h === n ? (r = h) : h !== f && (r = h + 1)
							}
				...
				} else if (Re(s) && t.charCodeAt(1) === Ne) {
				...
				for (; c >= r; --c) {
					...
					;(l === -1 && ((u = !1), (l = c + 1)),
						s === Be ? (i === -1 ? (i = c) : m !== 1 && (m = 1)) : i !== -1 && (m = -1))
				}
				return (
					l !== -1 &&
						(i === -1 || m === 0 || (m === 1 && i === l - 1 && i === o + 1)
							? (e.base = e.name = t.slice(o, l))
							: ((e.name = t.slice(o, i)), (e.base = t.slice(o, l)), (e.ext = t.slice(i, l)))),
					o > 0 && o !== r ? (e.dir = t.slice(0, o - 1)) : (e.dir = e.root),
					e
				)
			},
			sep: '\\',
		...
		ne = {
			resolve(...t) {
				...
				return ((e = Vt(e, !n, '/', Rn)), n ? `/${e}` : e.length > 0 ? e : '.')
			},
			normalize(t) {
				if ((J(t, 'path'), t.length === 0)) return '.'
				const e = t.charCodeAt(0) === Z,
					n = t.charCodeAt(t.length - 1) === Z
				return (
					(t = Vt(t, !e, '/', Rn)),
			...
			relative(t, e) {
				if (
					(J(t, 'from'), J(e, 'to'), t === e || ((t = ne.resolve(t)), (e = ne.resolve(e)), t === e))
				)
					return ''
				const n = 1,
					r = t.length,
				...
				for (; c < l; c++) {
					...
					if (h !== e.charCodeAt(i + c)) break
					h === Z && (u = c)
				}
				if (c === l)
					if (o > l) {
						if (e.charCodeAt(i + c) === Z) return e.slice(i + c + 1)
			...
			parse(t) {
				...
				for (; u >= r; --u) {
					...
					;(o === -1 && ((l = !1), (o = u + 1)),
						m === Be ? (s === -1 ? (s = u) : c !== 1 && (c = 1)) : s !== -1 && (c = -1))
				}
				if (o !== -1) {
					const m = i === 0 && n ? 1 : i
					s === -1 || c === 0 || (c === 1 && s === o - 1 && s === i + 1)
						? (e.base = e.name = t.slice(m, o))
						: ((e.name = t.slice(m, s)), (e.base = t.slice(m, o)), (e.ext = t.slice(s, o)))
				}
	...
	;((ne.win32 = te.win32 = te), (ne.posix = te.posix = ne), Se ? te.normalize : ne.normalize)
	const ka = Se ? te.join : ne.join
	;(Se ? te.resolve : ne.resolve,
		Se ? te.relative : ne.relative,
		Se ? te.dirname : ne.dirname,
		Se ? te.basename : ne.basename,
		Se ? te.extname : ne.extname,
		Se ? te.sep : ne.sep)
	const Ma = /^\w[\w\d+.-]*$/,
		Pa = /^\//,
		Ta = /^\/\//
	...
	class le {
		...
		with(e) {
			...
			return (
				n === void 0 ? (n = this.scheme) : n === null && (n = j),
				r === void 0 ? (r = this.authority) : r === null && (r = j),
				s === void 0 ? (s = this.path) : s === null && (s = j),
				i === void 0 ? (i = this.query) : i === null && (i = j),
				o === void 0 ? (o = this.fragment) : o === null && (o = j),
				n === this.scheme &&
				r === this.authority &&
				s === this.path &&
				i === this.query &&
		...
		static parse(e, n = !1) {
			const r = Va.exec(e)
			return r
				? new Je(r[2] || j, Bt(r[4] || j), Bt(r[5] || j), Bt(r[7] || j), Bt(r[9] || j), n)
				: new Je(j, j, j, j, j)
		}
		static file(e) {
			let n = j
			if ((dt && (e = e.replace(/\\/g, pe)), e[0] === pe && e[1] === pe)) {
				const r = e.indexOf(pe, 2)
				r === -1
					? ((n = e.substring(2)), (e = pe))
					: ((n = e.substring(2, r)), (e = e.substring(r) || pe))
			}
			return new Je('file', n, e, j, j)
		}
		static from(e, n) {
		...
		static joinPath(e, ...n) {
			...
			return (
				dt && e.scheme === 'file'
					? (r = le.file(te.join(Cn(e, !0), ...n)).path)
					: (r = ne.join(e.path, ...n)),
				e.with({ path: r })
			)
		}
			...
			if (e) {
				if (e instanceof le) return e
				{
					...
					return (
						...
						(n._fsPath = e._sep === yr ? (e.fsPath ?? null) : null),
						n
					)
				}
			} else return e
	...
	class Je extends le {
		...
		toString(e = !1) {
			return e
				? An(this, !0)
				: (this._formatted || (this._formatted = An(this, !1)), this._formatted)
		}
		toJSON() {
			const e = { $mid: 1 }
			return (
				this._fsPath && ((e.fsPath = this._fsPath), (e._sep = yr)),
	...
	function _r(t, e, n) {
		...
		for (let i = 0; i < t.length; i++) {
			...
			if (
				...
				const l = Lr[o]
				l !== void 0
					? (s !== -1 && ((r += encodeURIComponent(t.substring(s, i))), (s = -1)), (r += l))
					: s === -1 && (s = i)
			}
		}
	...
	function Ba(t) {
		...
		for (let n = 0; n < t.length; n++) {
			const r = t.charCodeAt(n)
			r === 35 || r === 63
				? (e === void 0 && (e = t.substr(0, n)), (e += Lr[r]))
				: e !== void 0 && (e += t[n])
		}
		return e !== void 0 ? e : t
	}
	...
	function An(t, e) {
		const n = e ? Ba : _r
		let r = '',
			{ scheme: s, authority: i, path: o, query: l, fragment: u } = t
		if ((s && ((r += s), (r += ':')), (i || s === 'file') && ((r += pe), (r += pe)), i)) {
			let c = i.indexOf('@')
			if (c !== -1) {
				...
				;((i = i.substr(c + 1)),
					...
					c === -1
						? (r += n(m, !1, !1))
						: ((r += n(m.substr(0, c), !1, !1)), (r += ':'), (r += n(m.substr(c + 1), !1, !0))),
					(r += '@'))
			}
			;((i = i.toLowerCase()),
				(c = i.lastIndexOf(':')),
	...
	function vr(t) {
		try {
			...
			return t.length > 3 ? t.substr(0, 3) + vr(t.substr(3)) : t
		}
	}
	const Nr = /(%[0-9A-Za-z][0-9A-Za-z])+/g
	function Bt(t) {
		return t.match(Nr) ? t.replace(Nr, (e) => vr(e)) : t
	}
	class ue extends R {
		constructor(e, n, r, s) {
			;(super(e, n, r, s),
		...
		setEndPosition(e, n) {
			return this.getDirection() === 0
				? new ue(this.startLineNumber, this.startColumn, e, n)
				: new ue(e, n, this.startLineNumber, this.startColumn)
		}
		getPosition() {
		...
		setStartPosition(e, n) {
			return this.getDirection() === 0
				? new ue(e, n, this.endLineNumber, this.endColumn)
				: new ue(this.endLineNumber, this.endColumn, e, n)
		}
		static fromPositions(e, n = e) {
		...
		static fromRange(e, n) {
			return n === 0
				? new ue(e.startLineNumber, e.startColumn, e.endLineNumber, e.endColumn)
				: new ue(e.endLineNumber, e.endColumn, e.startLineNumber, e.startColumn)
		}
	...
	;(function (t) {
		...
		function n(o) {
			let l = e.get(o)
			return (
				l || (console.info('No codicon found for CompletionItemKind ' + o), (l = P.symbolProperty)),
				l
			)
		}
		t.toIcon = n
	...
	;(function (t) {
		...
		function n(i) {
			let o = e.get(i)
			return (
				o || (console.info('No codicon found for SymbolKind ' + i), (o = P.symbolProperty)),
				o
			)
		}
		t.toIcon = n
		...
		function s(i) {
			let o = r.get(i)
			return (
				o === void 0 && (console.info('No completion kind found for SymbolKind ' + i), (o = 20)),
				o
			)
		}
	...
	function Za(t, e, n, r, s) {
		return Ja(t, e, n, r, s) && Ya(t, e, n, r, s)
	}
	class Ka {
		constructor(e, n) {
		...
		next(e) {
			...
			do {
				...
				if (s === this._prevMatchStartIndex && i === this._prevMatchLength) {
					if (i === 0) {
						Zi(e, n, this._searchRegex.lastIndex) > 65535
				...
				if (
					...
					!this._wordSeparators || Za(this._wordSeparators, e, n, s, i))
				)
					return r
			} while (r)
			return null
	...
	function to(t = '') {
		let e = '(-?\\d*\\.\\d\\w*)|([^'
		for (const n of eo) t.indexOf(n) >= 0 || (e += '\\' + n)
	...
	const Vs = new Ni()
	Vs.unshift({ maxLen: 1e3, windowSize: 15, timeBudget: 150 })
	function Fn(t, e, n, r, s) {
		if (((e = Is(e)), s || (s = Ct.first(Vs)), n.length > s.maxLen)) {
			let c = t - s.maxLen / 2
			return (c < 0 ? (c = 0) : (r += c), (n = n.substring(c, t + s.maxLen / 2)), Fn(t, e, n, r, s))
		}
		const i = Date.now(),
			o = t - 1 - r
		let l = -1,
	...
	class ro {
		static computeUnicodeHighlights(e, n, r) {
			...
			l === 'allNonBasicAscii'
				? (u = new RegExp('[^\\t\\n\\r\\x20-\\x7E]', 'g'))
				: (u = new RegExp(`${so(Array.from(l))}`, 'g'))
			const c = new Ka(null, u),
				m = []
			let h = !1,
				f,
				p = 0,
			...
			e: for (let w = s, _ = i; w <= _; w++) {
				...
				do
					if (((f = c.next(v)), f)) {
						...
						if (k > 0) {
							const S = v.charCodeAt(k - 1)
							bn(S) && k--
						}
						if (D + 1 < L) {
							const S = v.charCodeAt(D - 1)
							bn(S) && D++
						}
						const W = v.substring(k, D)
						let x = Fn(k + 1, Ds, v, 0)
						x && x.endColumn <= k + 1 && (x = null)
						const y = o.shouldHighlightNonBasicASCII(W, x ? x.word : null)
						if (y !== 0) {
							if ((y === 3 ? p++ : y === 2 ? g++ : y === 1 ? d++ : wi(), m.length >= 1e3)) {
								h = !0
								break e
							}
			...
			switch (r.shouldHighlightNonBasicASCII(e, null)) {
				...
				case 3: {
					const i = e.codePointAt(0),
						o = r.ambiguousCharacters.getPrimaryConfusable(i),
						l = gt
							.getLocales()
							.filter((u) => !gt.getInstance(new Set([...n.allowedLocales, u])).isAmbiguous(i))
	...
	function so(t, e) {
		return `[${Oi(t.map((r) => String.fromCodePoint(r)).join(''))}]`
	}
	class Bs {
		constructor(e) {
			;((this.options = e),
				(this.allowedCodePoints = new Set(e.allowedCodePoints)),
				(this.ambiguousCharacters = gt.getInstance(new Set(e.allowedLocales))))
		}
		getCandidateCodePoints() {
			if (this.options.nonBasicASCII) return 'allNonBasicAscii'
			const e = new Set()
			if (this.options.invisibleCharacters)
				for (const n of pt.codePoints) qs(String.fromCodePoint(n)) || e.add(n)
				...
				for (const o of n) {
					...
					;((s = s || u),
						!u &&
							...
							!pt.isInvisibleCharacter(l) &&
							(i = !0))
				}
			return !s && i
				? 0
				: this.options.invisibleCharacters && !qs(e) && pt.isInvisibleCharacter(r)
					? 2
...
` ||
	...
	const ut = class ut {
		...
		filter(e) {
			return new ut((n) => this.iterate((r) => (e(r) ? n(r) : !0)))
		}
		map(e) {
			return new ut((n) => this.iterate((r) => n(e(r))))
		}
		findLast(e) {
			let n
		...
		findLastMaxBy(e) {
			let n,
				r = !0
			return (this.iterate((s) => ((r || Dn.isGreaterThan(e(s, n))) && ((r = !1), (n = s)), !0)), n)
		}
	}
	ut.empty = new ut((e) => {})
	let $s = ut
		...
		static addRange(e, n) {
			...
			if (r === s) n.splice(r, 0, e)
				...
				const i = Math.min(e.start, n[r].start),
					o = Math.max(e.endExclusive, n[s - 1].endExclusive)
				n.splice(r, s - r, new T(i, o))
			}
		}
		static tryCreate(e, n) {
			if (!(e > n)) return new T(e, n)
		}
		static ofLength(e) {
			return new T(0, e)
		}
	...
	function In(t, e, n = 0, r = t.length) {
		let s = n,
			i = r
		for (; s < i; ) {
			const o = Math.floor((s + i) / 2)
	...
	const me = class me {
		...
		static joinMany(e) {
			...
			for (let r = 1; r < e.length; r++) n = n.getUnion(new ye(e[r].slice()))
			return n.ranges
		}
		static join(e) {
		...
		toInclusiveRange() {
			return this.isEmpty
				? null
				: new R(this.startLineNumber, 1, this.endLineNumberExclusive - 1, Number.MAX_SAFE_INTEGER)
		}
		toExclusiveRange() {
	...
	me.compareByStart = Ye((e) => e.startLineNumber, xt)
	let z = me
	class ye {
		constructor(e = []) {
		...
		addRange(e) {
			if (e.length === 0) return
			const n = In(this._normalizedRanges, (s) => s.endLineNumberExclusive >= e.startLineNumber),
				r = Ke(this._normalizedRanges, (s) => s.startLineNumber <= e.endLineNumberExclusive) + 1
		...
		getUnion(e) {
			...
			for (; r < this._normalizedRanges.length || s < e._normalizedRanges.length; ) {
				...
				if (r < this._normalizedRanges.length && s < e._normalizedRanges.length) {
					const l = this._normalizedRanges[r],
			...
			return (i !== null && n.push(i), new ye(n))
		}
		subtractFrom(e) {
			const n = In(this._normalizedRanges, (o) => o.endLineNumberExclusive >= e.startLineNumber),
				r = Ke(this._normalizedRanges, (o) => o.startLineNumber <= e.endLineNumberExclusive) + 1
			if (n === r) return new ye([e])
			const s = []
			let i = e.startLineNumber
			for (let o = n; o < r; o++) {
				const l = this._normalizedRanges[o]
				;(l.startLineNumber > i && s.push(new z(i, l.startLineNumber)),
					(i = l.endLineNumberExclusive))
			}
			return (i < e.endLineNumberExclusive && s.push(new z(i, e.endLineNumberExclusive)), new ye(s))
		}
		toString() {
		...
		getIntersection(e) {
			...
			for (; r < this._normalizedRanges.length && s < e._normalizedRanges.length; ) {
				const i = this._normalizedRanges[r],
					o = e._normalizedRanges[s],
					l = i.intersect(o)
				;(l && !l.isEmpty && n.push(l),
					i.endLineNumberExclusive < o.endLineNumberExclusive ? r++ : s++)
			}
			return new ye(n)
		}
		getWithDelta(e) {
	...
	const we = class we {
		static betweenPositions(e, n) {
			return e.lineNumber === n.lineNumber
				? new we(0, n.column - e.column)
				: new we(n.lineNumber - e.lineNumber, n.column - 1)
		}
		static fromPosition(e) {
			return new we(e.lineNumber - 1, e.column - 1)
		}
		static ofRange(e) {
			return we.betweenPositions(e.getStartPosition(), e.getEndPosition())
		}
		static ofText(e) {
			let n = 0,
				r = 0
...
`
		...
		add(e) {
			return e.lineCount === 0
				? new we(this.lineCount, this.columnCount + e.columnCount)
				: new we(this.lineCount + e.lineCount, e.columnCount)
		}
		createRange(e) {
			return this.lineCount === 0
				? new R(e.lineNumber, e.column, e.lineNumber, e.column + this.columnCount)
				: new R(e.lineNumber, e.column, e.lineNumber + this.lineCount, this.columnCount + 1)
		}
		toRange() {
	...
	class mo {
		...
		getStringReplacement(e) {
			return new qe.deps.StringReplacement(this.getOffsetRange(e.range), e.text)
		}
		getTextReplacement(e) {
			return new qe.deps.TextReplacement(this.getRange(e.replaceRange), e.newText)
		}
		getTextEdit(e) {
...
` &&
		...
		getPosition(e) {
			const n = Ke(this.lineStartOffsetByLineIdx, (i) => i <= e),
				r = n + 1,
		...
		get textLength() {
			...
			return new qe.deps.TextLength(e, this.text.length - this.lineStartOffsetByLineIdx[e])
		}
		getLineLength(e) {
	...
	class go extends Hs {
		constructor(e, n) {
			;(xi(n >= 1), super(), (this._getLineContent = e), (this._lineCount = n))
		}
		getValueOfRange(e) {
...
` + this._getLineContent(e.endLineNumber).substring(0, e.endColumn - 1)),
		...
		get length() {
			...
			return new Pe(this._lineCount - 1, e.length)
		}
	}
	class Ut extends go {
		constructor(e) {
	...
	class $t extends Hs {
		constructor(e) {
			;(super(), (this.value = e), (this._t = new Ws(this.value)))
		}
		getValueOfRange(e) {
	...
	class Wt {
		static fromStringEdit(e, n) {
			const r = e.replacements.map((s) => be.fromStringReplacement(s, n))
			return new Wt(r)
		}
		static fromParallelReplacementsUnsorted(e) {
			const n = e.slice().sort(Ye((r) => r.range, R.compareRangesUsingStarts))
			return new Wt(n)
		}
		constructor(e) {
			;((this.replacements = e),
				ht(() =>
					tn(e, (n, r) => n.range.getEndPosition().isBeforeOrEqual(r.range.getStartPosition())),
				))
		}
		mapPosition(e) {
			let n = 0,
			...
			for (const i of this.replacements) {
				...
				const l = i.range.getEndPosition(),
					u = Pe.ofText(i.text)
				if (e.isBefore(l)) {
					const c = new q(o.lineNumber + n, o.column + (o.lineNumber + n === r ? s : 0)),
						m = u.addToPosition(c)
					return Ht(c, m)
				}
				;(o.lineNumber + n !== r && (s = 0),
					(n += u.lineCount - (i.range.endLineNumber - i.range.startLineNumber)),
					u.lineCount === 0
						? l.lineNumber !== o.lineNumber
							? (s += u.columnCount - (l.column - 1))
							: (s += u.columnCount - (l.column - o.column))
						: (s = u.columnCount),
					(r = l.lineNumber + n))
			}
			return new q(e.lineNumber + n, e.column + (e.lineNumber + n === r ? s : 0))
		}
		mapRange(e) {
			function n(o) {
				return o instanceof q ? o : o.getStartPosition()
			}
			function r(o) {
				return o instanceof q ? o : o.getEndPosition()
			}
			const s = n(this.mapPosition(e.getStartPosition())),
				i = r(this.mapPosition(e.getEndPosition()))
			return Ht(s, i)
		}
		apply(e) {
			let n = '',
			...
			for (const i of this.replacements) {
				const o = i.range,
					l = o.getStartPosition(),
					u = o.getEndPosition(),
					c = Ht(r, l)
				;(c.isEmpty() || (n += e.getValueOfRange(c)), (n += i.text), (r = u))
			}
			const s = Ht(r, e.endPositionExclusive)
		...
		getNewRanges() {
			...
			for (const i of this.replacements) {
				const o = Pe.ofText(i.text),
					...
					u = o.createRange(l)
				;(e.push(u),
					(r = u.endLineNumber - i.range.endLineNumber),
					(s = u.endColumn - i.range.endColumn),
					(n = i.range.endLineNumber))
			}
			return e
		}
		toReplacement(e) {
			...
			for (let i = 0; i < this.replacements.length; i++) {
				...
				if (((s += o.text), i < this.replacements.length - 1)) {
					const l = this.replacements[i + 1],
						u = R.fromPositions(o.range.getEndPosition(), l.range.getStartPosition()),
						c = e.getValueOfRange(u)
					s += c
				}
			}
			return new be(R.fromPositions(n, r), s)
		}
		toString(e) {
			return e === void 0
		...
		static joinReplacements(e, n) {
			...
			for (let o = 0; o < e.length; o++) {
				...
				if (((i += l.text), o < e.length - 1)) {
					const u = e[o + 1],
						c = R.fromPositions(l.range.getEndPosition(), u.range.getStartPosition()),
						m = n.getValueOfRange(c)
					i += m
				}
			}
			return new be(R.fromPositions(r, s), i)
		}
		static fromStringReplacement(e, n) {
			return new be(n.getTransformer().getRange(e.replaceRange), e.newText)
		}
		static delete(e) {
			return new be(e, '')
		}
...
`,
				...
				s = gn(n, r),
				i = Pe.ofText(n.substring(0, s)).addToPosition(this.range.getStartPosition()),
				o = r.substring(s),
				l = R.fromPositions(i, this.range.getEndPosition())
			return new be(l, o)
		}
...
`,
				...
				s = pn(n, r),
				i = Pe.ofText(n.substring(0, n.length - s)).addToPosition(this.range.getStartPosition()),
				o = r.substring(0, r.length - s),
				l = R.fromPositions(this.range.getStartPosition(), i)
			return new be(l, o)
		}
		toString() {
	...
	function Ht(t, e) {
		...
		if (!t.isBeforeOrEqual(e)) throw new Y('start must be before end')
	...
	class ce {
		static inverse(e, n, r) {
			...
			let i = 1,
				o = 1
			for (const u of e) {
				const c = new ce(new z(i, u.original.startLineNumber), new z(o, u.modified.startLineNumber))
				;(c.modified.isEmpty || s.push(c),
					(i = u.original.endLineNumberExclusive),
					(o = u.modified.endLineNumberExclusive))
			}
			const l = new ce(new z(i, n + 1), new z(o, r + 1))
		...
		static clip(e, n, r) {
			...
			for (const i of e) {
				const o = i.original.intersect(n),
					l = i.modified.intersect(r)
				o && !o.isEmpty && l && !l.isEmpty && s.push(new ce(o, l))
			}
			return s
		}
		constructor(e, n) {
		...
		join(e) {
			return new ce(this.original.join(e.original), this.modified.join(e.modified))
		}
		toRangeMapping() {
			const e = this.original.toInclusiveRange(),
				n = this.modified.toInclusiveRange()
			if (e && n) return new he(e, n)
			if (this.original.startLineNumber === 1 || this.modified.startLineNumber === 1) {
				if (!(this.modified.startLineNumber === 1 && this.original.startLineNumber === 1))
					throw new Y('not a valid diff')
				return new he(
					new R(this.original.startLineNumber, 1, this.original.endLineNumberExclusive, 1),
					new R(this.modified.startLineNumber, 1, this.modified.endLineNumberExclusive, 1),
				)
			} else
				return new he(
					new R(
		...
		toRangeMapping2(e, n) {
			if (
				zs(this.original.endLineNumberExclusive, e) &&
			...
			if (!this.original.isEmpty && !this.modified.isEmpty)
				return new he(
					R.fromPositions(
						...
						et(new q(this.original.endLineNumberExclusive - 1, Number.MAX_SAFE_INTEGER), e),
					),
					R.fromPositions(
						new q(this.modified.startLineNumber, 1),
						et(new q(this.modified.endLineNumberExclusive - 1, Number.MAX_SAFE_INTEGER), n),
					),
				)
			if (this.original.startLineNumber > 1 && this.modified.startLineNumber > 1)
				return new he(
					R.fromPositions(
						et(new q(this.original.startLineNumber - 1, Number.MAX_SAFE_INTEGER), e),
						et(new q(this.original.endLineNumberExclusive - 1, Number.MAX_SAFE_INTEGER), e),
					),
					R.fromPositions(
						et(new q(this.modified.startLineNumber - 1, Number.MAX_SAFE_INTEGER), n),
						et(new q(this.modified.endLineNumberExclusive - 1, Number.MAX_SAFE_INTEGER), n),
					),
				)
			throw new Y()
	...
	class Ce extends ce {
		static fromRangeMappings(e) {
			const n = z.join(e.map((s) => z.fromRangeInclusive(s.originalRange))),
				r = z.join(e.map((s) => z.fromRangeInclusive(s.modifiedRange)))
			return new Ce(n, r, e)
		}
		constructor(e, n, r) {
	...
	class he {
		static fromEdit(e) {
			...
			return e.replacements.map((s, i) => new he(s.range, n[i]))
		}
	...
	function Vn(t, e, n, r = !1) {
		...
		for (const i of ao(
			t.map((o) => po(o, e, n)),
			(o, l) =>
				o.original.intersectsOrTouches(l.original) || o.modified.intersectsOrTouches(l.modified),
		)) {
			const o = i[0],
				l = i[i.length - 1]
			s.push(
			...
			ht(() =>
				...
				(s[0].modified.startLineNumber !== s[0].original.startLineNumber ||
					...
					: tn(
							s,
							(i, o) =>
								o.original.startLineNumber - i.original.endLineNumberExclusive ===
									o.modified.startLineNumber - i.modified.endLineNumberExclusive &&
								i.original.endLineNumberExclusive < o.original.startLineNumber &&
								i.modified.endLineNumberExclusive < o.modified.startLineNumber,
						),
			),
			s
		)
	...
	function po(t, e, n) {
		...
		;(t.modifiedRange.endColumn === 1 &&
			...
			t.modifiedRange.startColumn - 1 >= n.getLineLength(t.modifiedRange.startLineNumber) &&
				...
				(r = 1))
		const i = new z(t.originalRange.startLineNumber + r, t.originalRange.endLineNumber + 1 + s),
			o = new z(t.modifiedRange.startLineNumber + r, t.modifiedRange.endLineNumber + 1 + s)
		return new Ce(i, o, [t])
	}
	const bo = 3
	class wo {
		computeDiff(e, n, r) {
			...
			for (const u of i.changes) {
				...
				u.modifiedEndLineNumber === 0
					? (m = new z(u.modifiedStartLineNumber + 1, u.modifiedStartLineNumber + 1))
					: (m = new z(u.modifiedStartLineNumber, u.modifiedEndLineNumber + 1))
				let h = new Ce(
					c,
					m,
					u.charChanges?.map(
				...
				;(l &&
					...
					((h = new Ce(
						...
						l.innerChanges && h.innerChanges ? l.innerChanges.concat(h.innerChanges) : void 0,
					)),
					o.pop()),
					o.push(h),
					(l = h))
			...
			return (
				...
				new wt(o, [], i.quitEarly)
			)
		}
	}
	function Os(t, e, n, r) {
		return new Me(t, e, n).ComputeDiff(r)
	}
	let js = class {
		constructor(e) {
			const n = [],
				r = []
			for (let s = 0, i = e.length; s < i; s++) ((n[s] = Bn(e[s], 1)), (r[s] = qn(e[s], 1)))
		...
		createCharSequence(e, n, r) {
			...
			for (let u = n; u <= r; u++) {
				const c = this.lines[u],
					m = e ? this._startColumns[u] : 1,
					h = e ? this._endColumns[u] : c.length + 1
				for (let f = m; f < h; f++) ((s[l] = c.charCodeAt(f - 1)), (i[l] = u + 1), (o[l] = f), l++)
	...
	class tt {
		...
		static createFromDiffChange(e, n, r) {
			const s = n.getStartLineNumber(e.originalStart),
				...
				o = n.getEndLineNumber(e.originalStart + e.originalLength - 1),
				...
				m = r.getEndLineNumber(e.modifiedStart + e.modifiedLength - 1),
				h = r.getEndColumn(e.modifiedStart + e.modifiedLength - 1)
			return new tt(s, i, o, l, u, c, m, h)
		}
	}
	function yo(t) {
	...
	class yt {
		...
		static createFromDiffResult(e, n, r, s, i, o, l) {
			...
			if (
				...
				n.modifiedLength === 0
					? ((m = s.getStartLineNumber(n.modifiedStart) - 1), (h = 0))
					: ((m = s.getStartLineNumber(n.modifiedStart)),
				...
				if (p.getElements().length > 0 && g.getElements().length > 0) {
					let d = Os(p, g, i, !0).changes
					;(l && (d = yo(d)), (f = []))
					for (let w = 0, _ = d.length; w < _; w++) f.push(tt.createFromDiffChange(d[w], p, g))
				}
			}
			return new yt(u, c, m, h, f)
		}
	}
	class Lo {
		constructor(e, n, r) {
			;((this.shouldComputeCharChanges = r.shouldComputeCharChanges),
				...
				(this.original = new js(e)),
				(this.modified = new js(n)),
				(this.continueLineDiff = Gs(r.maxComputationTime)),
				(this.continueCharDiff = Gs(
		...
		computeDiff() {
			...
			const e = Os(this.original, this.modified, this.continueLineDiff, this.shouldMakePrettyDiff),
				n = e.changes,
				r = e.quitEarly
			...
			for (let l = -1, u = n.length; l < u; l++) {
				const c = l + 1 < u ? n[l + 1] : null,
					m = c ? c.originalStart : this.originalLines.length,
					h = c ? c.modifiedStart : this.modifiedLines.length
				for (; i < m && o < h; ) {
					const f = this.originalLines[i],
					...
					if (f !== p) {
						...
						}
						{
							...
							const w = f.length + 1,
								_ = p.length + 1
							for (; g < w && d < _; ) {
								const v = f.charCodeAt(g - 1),
									L = f.charCodeAt(d - 1)
								if (v !== L) break
								;(g++, d++)
							}
							;(g < w || d < _) && this._pushTrimWhitespaceCharChange(s, i + 1, g, w, o + 1, d, _)
						}
					}
					;(i++, o++)
				}
		...
		_pushTrimWhitespaceCharChange(e, n, r, s, i, o, l) {
			if (this._mergeTrimWhitespaceCharChange(e, n, r, s, i, o, l)) return
			let u
			;(this.shouldComputeCharChanges && (u = [new tt(n, r, n, s, i, o, i, l)]),
				e.push(new yt(n, n, i, i, u)))
		}
		_mergeTrimWhitespaceCharChange(e, n, r, s, i, o, l) {
			...
			return c.originalEndLineNumber === 0 || c.modifiedEndLineNumber === 0
				...
				: c.originalEndLineNumber === n && c.modifiedEndLineNumber === i
					? (this.shouldComputeCharChanges &&
							c.charChanges &&
							c.charChanges.push(new tt(n, r, n, s, i, o, i, l)),
						!0)
	...
	class Ae {
		static trivial(e, n) {
			return new Ae([new Q(T.ofLength(e.length), T.ofLength(n.length))], !1)
		}
		static trivialTimedOut(e, n) {
			return new Ae([new Q(T.ofLength(e.length), T.ofLength(n.length))], !0)
		}
		constructor(e, n) {
	...
	class Q {
		static invert(e, n) {
			...
			return (
				oo(e, (s, i) => {
					r.push(
						Q.fromOffsetPairs(
							s ? s.getEndExclusives() : Ee.zero,
							i
								? i.getStarts()
								: new Ee(n, (s ? s.seq2Range.endExclusive - s.seq1Range.endExclusive : 0) + n),
						),
					)
				}),
				r
		...
		getStarts() {
			return new Ee(this.seq1Range.start, this.seq2Range.start)
		}
		getEndExclusives() {
			return new Ee(this.seq1Range.endExclusive, this.seq2Range.endExclusive)
		}
	}
	const Ue = class Ue {
		constructor(e, n) {
		...
		delta(e) {
			return e === 0 ? this : new Ue(this.offset1 + e, this.offset2 + e)
		}
		equals(e) {
			return this.offset1 === e.offset1 && this.offset2 === e.offset2
		}
	}
	;((Ue.zero = new Ue(0, 0)), (Ue.max = new Ue(Number.MAX_SAFE_INTEGER, Number.MAX_SAFE_INTEGER)))
	let Ee = Ue
	const Zt = class Zt {
		isValid() {
			return !0
...
`)
	...
	class vo {
		compute(e, n, r = Lt.instance, s) {
			if (e.length === 0 || n.length === 0) return Ae.trivial(e, n)
			const i = new Un(e.length, n.length),
				o = new Un(e.length, n.length),
				l = new Un(e.length, n.length)
			for (let g = 0; g < e.length; g++)
				for (let d = 0; d < n.length; d++) {
					if (!r.isValid()) return Ae.trivialTimedOut(e, n)
					const w = g === 0 ? 0 : i.get(g - 1, d),
						_ = d === 0 ? 0 : i.get(g, d - 1)
					let v
					e.getElement(g) === n.getElement(d)
						? (g === 0 || d === 0 ? (v = 0) : (v = i.get(g - 1, d - 1)),
							g > 0 && d > 0 && o.get(g - 1, d - 1) === 3 && (v += l.get(g - 1, d - 1)),
							(v += s ? s(g, d) : 1))
						: (v = -1)
					const L = Math.max(w, _, v)
					if (L === v) {
						const k = g > 0 && d > 0 ? l.get(g - 1, d - 1) : 0
						;(l.set(g, d, k + 1), o.set(g, d, 3))
					} else
						L === w ? (l.set(g, d, 0), o.set(g, d, 1)) : L === _ && (l.set(g, d, 0), o.set(g, d, 2))
					i.set(g, d, L)
				}
			const u = []
			let c = e.length,
				m = n.length
			function h(g, d) {
				;((g + 1 !== c || d + 1 !== m) && u.push(new Q(new T(g + 1, c), new T(d + 1, m))),
					(c = g),
					(m = d))
			}
			let f = e.length - 1,
				p = n.length - 1
			for (; f >= 0 && p >= 0; )
				o.get(f, p) === 3 ? (h(f, p), f--, p--) : o.get(f, p) === 1 ? f-- : p--
			return (h(-1, -1), u.reverse(), new Ae(u, !1))
		}
	}
	class Xs {
		compute(e, n, r = Lt.instance) {
			if (e.length === 0 || n.length === 0) return Ae.trivial(e, n)
			const s = e,
				i = n
			function o(d, w) {
				for (; d < s.length && w < i.length && s.getElement(d) === i.getElement(w); ) (d++, w++)
				return d
			}
			let l = 0
			const u = new No()
			u.set(0, o(0, 0))
			const c = new So()
			c.set(0, u.get(0) === 0 ? null : new Qs(null, 0, 0, u.get(0)))
			let m = 0
			e: for (;;) {
				if ((l++, !r.isValid())) return Ae.trivialTimedOut(s, i)
				const d = -Math.min(l, i.length + (l % 2)),
					w = Math.min(l, s.length + (l % 2))
				for (m = d; m <= w; m += 2) {
					const _ = m === w ? -1 : u.get(m + 1),
						v = m === d ? -1 : u.get(m - 1) + 1,
						L = Math.min(Math.max(_, v), s.length),
						k = L - m
					if (L > s.length || k > i.length) continue
					const D = o(L, k)
					u.set(m, D)
					const W = L === _ ? c.get(m + 1) : c.get(m - 1)
					if (
						(c.set(m, D !== L ? new Qs(W, L, k, D - L) : W),
						u.get(m) === s.length && u.get(m) - m === i.length)
					)
						break e
				}
			}
			...
			let p = s.length,
				g = i.length
			for (;;) {
				const d = h ? h.x + h.length : 0,
					w = h ? h.y + h.length : 0
				if (((d !== p || w !== g) && f.push(new Q(new T(d, p), new T(w, g))), !h)) break
				;((p = h.x), (g = h.y), (h = h.prev))
			}
			return (f.reverse(), new Ae(f, !1))
		}
	}
	class Qs {
		constructor(e, n, r, s) {
	...
	class Ot {
		...
		getBoundaryScore(e) {
			const n = Zs(e > 0 ? this.elements[e - 1] : -1),
				r = Zs(e < this.elements.length ? this.elements[e] : -1)
			if (n === 7 && r === 8) return 0
			if (n === 8) return 150
			let s = 0
			return (n !== r && ((s += 10), n === 0 && r === 1 && (s += 1)), (s += Ys(n)), (s += Ys(r)), s)
		}
		translateOffset(e, n = 'right') {
			const r = Ke(this.firstElementOffsetByLineIdx, (i) => i <= e),
		...
		findSubWordContaining(e) {
			if (e < 0 || e >= this.elements.length || !nt(this.elements[e])) return
			let n = e
			for (; n > 0 && nt(this.elements[n - 1]) && !Js(this.elements[n]); ) n--
			let r = e
			for (; r < this.elements.length && nt(this.elements[r]) && !Js(this.elements[r]); ) r++
			return new T(n, r)
		}
		countLinesIn(e) {
			return (
		...
		extendToFullLines(e) {
			const n = Ze(this.firstElementOffsetByLineIdx, (s) => s <= e.start) ?? 0,
				r = ho(this.firstElementOffsetByLineIdx, (s) => e.endExclusive <= s) ?? this.elements.length
			return new T(n, r)
		}
	}
	function nt(t) {
	...
	const Ro = { 0: 0, 1: 0, 2: 0, 3: 10, 4: 2, 5: 30, 6: 3, 7: 10, 8: 10 }
	function Ys(t) {
		return Ro[t]
	}
	function Zs(t) {
	...
	function Co(t, e, n, r, s, i) {
		let { moves: o, excludedChanges: l } = Eo(t, e, n, i)
		if (!i.isValid()) return []
		const u = t.filter((m) => !l.has(m)),
			c = ko(u, r, s, e, n, i)
		return (
			uo(o, c),
			(o = Mo(o)),
...
`).length >= 15 && Ao(h, (p) => p.length >= 2) >= 2
	...
	function Eo(t, e, n, r) {
		const s = [],
			i = t
				...
				.map((u) => new zt(u.original, e, u)),
			o = new Set(
				t
					.filter((u) => u.original.isEmpty && u.modified.length >= 3)
					.map((u) => new zt(u.modified, n, u)),
			),
			l = new Set()
		for (const u of i) {
			let c = -1,
			...
			if (
				(c > 0.9 &&
					m &&
					(o.delete(m), s.push(new ce(u.range, m.range)), l.add(u.source), l.add(m.source)),
				!r.isValid())
			)
	...
	function ko(t, e, n, r, s, i) {
		const o = [],
			l = new Xa()
		for (const f of t)
			for (let p = f.original.startLineNumber; p < f.original.endLineNumberExclusive - 2; p++) {
				const g = `${e[p - 1]}:${e[p + 1 - 1]}:${e[p + 2 - 1]}`
				l.add(g, { range: new z(p, p + 3) })
			}
		const u = []
		t.sort(Ye((f) => f.modified.startLineNumber, xt))
		for (const f of t) {
			let p = []
			for (let g = f.modified.startLineNumber; g < f.modified.endLineNumberExclusive - 2; g++) {
				...
				;(l.forEach(d, ({ range: v }) => {
					for (const k of p)
						if (
							k.originalLineRange.endLineNumberExclusive + 1 === v.endLineNumberExclusive &&
							k.modifiedLineRange.endLineNumberExclusive + 1 === w.endLineNumberExclusive
						) {
							;((k.originalLineRange = new z(
								k.originalLineRange.startLineNumber,
								v.endLineNumberExclusive,
							)),
								(k.modifiedLineRange = new z(
					...
					const L = { modifiedLineRange: w, originalLineRange: v }
					;(u.push(L), _.push(L))
				}),
					(p = _))
			}
		...
		u.sort(co(Ye((f) => f.modifiedLineRange.length, xt)))
		const c = new ye(),
			m = new ye()
		for (const f of u) {
			const p = f.modifiedLineRange.startLineNumber - f.originalLineRange.startLineNumber,
				g = c.subtractFrom(f.modifiedLineRange),
				d = m.subtractFrom(f.originalLineRange).getWithDelta(p),
				w = g.getIntersection(d)
			for (const _ of w.ranges) {
				if (_.length < 3) continue
				const v = _,
					L = _.delta(-p)
				;(o.push(new ce(L, v)), c.addRange(v), m.addRange(L))
			}
		}
		o.sort(Ye((f) => f.original.startLineNumber, xt))
		const h = new qt(t)
		for (let f = 0; f < o.length; f++) {
			const p = o[f],
				g = h.findLastMonotonous((W) => W.original.startLineNumber <= p.original.startLineNumber),
				d = Ze(t, (W) => W.modified.startLineNumber <= p.modified.startLineNumber),
				w = Math.max(
					p.original.startLineNumber - g.original.startLineNumber,
					p.modified.startLineNumber - d.modified.startLineNumber,
				),
				_ = h.findLastMonotonous(
					(W) => W.original.startLineNumber < p.original.endLineNumberExclusive,
				),
				v = Ze(t, (W) => W.modified.startLineNumber < p.modified.endLineNumberExclusive),
				L = Math.max(
					_.original.endLineNumberExclusive - p.original.endLineNumberExclusive,
					v.modified.endLineNumberExclusive - p.modified.endLineNumberExclusive,
				)
			let k
			for (k = 0; k < w; k++) {
				const W = p.original.startLineNumber - k - 1,
					x = p.modified.startLineNumber - k - 1
				if (
					W > r.length ||
					x > s.length ||
					c.contains(x) ||
			...
			k > 0 &&
				(m.addRange(new z(p.original.startLineNumber - k, p.original.startLineNumber)),
				c.addRange(new z(p.modified.startLineNumber - k, p.modified.startLineNumber)))
			let D
			for (D = 0; D < L; D++) {
				const W = p.original.endLineNumberExclusive + D,
					x = p.modified.endLineNumberExclusive + D
				if (
					W > r.length ||
					x > s.length ||
					c.contains(x) ||
			...
			;(D > 0 &&
				(m.addRange(
					new z(p.original.endLineNumberExclusive, p.original.endLineNumberExclusive + D),
				),
				c.addRange(
					new z(p.modified.endLineNumberExclusive, p.modified.endLineNumberExclusive + D),
				)),
				(k > 0 || D > 0) &&
					(o[f] = new ce(
						new z(p.original.startLineNumber - k, p.original.endLineNumberExclusive + D),
						new z(p.modified.startLineNumber - k, p.modified.endLineNumberExclusive + D),
					)))
		}
		return o
	}
	function Ks(t, e, n) {
		...
		const s = new Xs().compute(
			new Ot([t], new R(1, 1, 1, t.length), !1),
			new Ot([e], new R(1, 1, 1, e.length), !1),
			n,
		)
		let i = 0
		...
		function l(m) {
			let h = 0
			for (let f = 0; f < t.length; f++) $n(m.charCodeAt(f)) || h++
			return h
		}
	...
	function Mo(t) {
		...
		t.sort(Ye((n) => n.original.startLineNumber, xt))
		const e = [t[0]]
			...
			(e = e.filter((r) => {
				const s =
						n.findLastMonotonous(
							(l) => l.original.startLineNumber < r.original.endLineNumberExclusive,
						) || new ce(new z(1, 1), new z(1, 1)),
					i = Ze(t, (l) => l.modified.startLineNumber < r.modified.endLineNumberExclusive)
				return s !== i
			})),
			e
		)
	...
	function ei(t, e, n) {
		let r = n
		return ((r = ti(t, e, r)), (r = ti(t, e, r)), (r = To(t, e, r)), r)
	}
	function ti(t, e, n) {
		...
		for (let i = 1; i < n.length; i++) {
			...
			if (l.seq1Range.isEmpty || l.seq2Range.isEmpty) {
				...
				for (
					...
					c <= u &&
					!(
						t.getElement(l.seq1Range.start - c) !== t.getElement(l.seq1Range.endExclusive - c) ||
						e.getElement(l.seq2Range.start - c) !== e.getElement(l.seq2Range.endExclusive - c)
					);
					c++
				);
				if ((c--, c === u)) {
					r[r.length - 1] = new Q(
						new T(o.seq1Range.start, l.seq1Range.endExclusive - u),
						new T(o.seq2Range.start, l.seq2Range.endExclusive - u),
					)
					continue
				}
				l = l.delta(-c)
		...
		for (let i = 0; i < r.length - 1; i++) {
			...
			if (l.seq1Range.isEmpty || l.seq2Range.isEmpty) {
				...
				for (
					...
					c < u &&
					!(
						!t.isStronglyEqual(l.seq1Range.start + c, l.seq1Range.endExclusive + c) ||
						!e.isStronglyEqual(l.seq2Range.start + c, l.seq2Range.endExclusive + c)
					);
					c++
				);
				if (c === u) {
					r[i + 1] = new Q(
						new T(l.seq1Range.start + u, o.seq1Range.endExclusive),
						new T(l.seq2Range.start + u, o.seq2Range.endExclusive),
					)
					continue
				}
				c > 0 && (l = l.delta(c))
	...
	function To(t, e, n) {
		...
		for (let r = 0; r < n.length; r++) {
			const s = r > 0 ? n[r - 1] : void 0,
				...
				l = new T(s ? s.seq1Range.endExclusive + 1 : 0, o ? o.seq1Range.start - 1 : t.length),
				u = new T(s ? s.seq2Range.endExclusive + 1 : 0, o ? o.seq2Range.start - 1 : e.length)
			i.seq1Range.isEmpty
				? (n[r] = ni(i, t, e, l, u))
				: i.seq2Range.isEmpty && (n[r] = ni(i.swap(), e, t, u, l).swap())
		}
		return n
	}
	function ni(t, e, n, r, s) {
		...
		for (
			;
			t.seq1Range.start - o >= r.start &&
			t.seq2Range.start - o >= s.start &&
		...
		for (
			;
			t.seq1Range.start + l < r.endExclusive &&
		...
		let u = 0,
			c = -1
		for (let m = -o; m <= l; m++) {
			const h = t.seq2Range.start + m,
				f = t.seq2Range.endExclusive + m,
				p = t.seq1Range.start + m,
				g = e.getBoundaryScore(p) + n.getBoundaryScore(h) + n.getBoundaryScore(f)
			g > c && ((c = g), (u = m))
		}
		return t.delta(u)
	}
	function Fo(t, e, n) {
		...
		for (const s of n) {
			...
			s.seq2Range.start - i.seq2Range.endExclusive <= 2
				? (r[r.length - 1] = new Q(i.seq1Range.join(s.seq1Range), i.seq2Range.join(s.seq2Range)))
				: r.push(s)
		}
		return r
	}
	function ri(t, e, n, r, s = !1) {
		const i = Q.invert(n, t.length),
			o = []
		let l = new Ee(0, 0)
		function u(m, h) {
			if (m.offset1 < l.offset1 || m.offset2 < l.offset2) return
			const f = r(t, m.offset1),
				p = r(e, m.offset2)
			if (!f || !p) return
			let g = new Q(f, p)
			const d = g.intersect(h)
			let w = d.seq1Range.length,
				_ = d.seq2Range.length
			for (; i.length > 0; ) {
				const v = i[0]
				if (!(v.seq1Range.intersects(g.seq1Range) || v.seq2Range.intersects(g.seq2Range))) break
				const k = r(t, v.seq1Range.start),
					D = r(e, v.seq2Range.start),
					W = new Q(k, D),
					x = W.intersect(v)
				if (
					((w += x.seq1Range.length),
					(_ += x.seq2Range.length),
					(g = g.join(W)),
			...
			;(((s && w + _ < g.seq1Range.length + g.seq2Range.length) ||
				w + _ < ((g.seq1Range.length + g.seq2Range.length) * 2) / 3) &&
				o.push(g),
				(l = g.getEndExclusives()))
		}
		for (; i.length > 0; ) {
			const m = i.shift()
	...
	function Io(t, e, n) {
		...
		do {
			...
			for (let l = 1; l < r.length; l++) {
				let m = function (f, p) {
					const g = new T(c.seq1Range.endExclusive, u.seq1Range.start)
					return (
				...
				const u = r[l],
					c = o[o.length - 1]
				m(c, u) ? ((i = !0), (o[o.length - 1] = o[o.length - 1].join(u))) : o.push(u)
			}
			r = o
		} while (s++ < 10 && i)
		return r
	...
	function Vo(t, e, n) {
		...
		do {
			...
			for (let u = 1; u < r.length; u++) {
				let h = function (p, g) {
					...
					const _ = t.getText(d).trim()
					if (_.length > 20 || _.split(/\r\n|\r|\n/).length > 1) return !1
					const v = t.countLinesIn(p.seq1Range),
						L = p.seq1Range.length,
						k = e.countLinesIn(p.seq2Range),
						D = p.seq2Range.length,
						W = t.countLinesIn(g.seq1Range),
						x = g.seq1Range.length,
						y = e.countLinesIn(g.seq2Range),
						S = g.seq2Range.length,
						V = 130
					function U(B) {
						return Math.min(B, V)
					}
					return (
						Math.pow(Math.pow(U(v * 40 + L), 1.5) + Math.pow(U(k * 40 + D), 1.5), 1.5) +
							Math.pow(Math.pow(U(W * 40 + x), 1.5) + Math.pow(U(y * 40 + S), 1.5), 1.5) >
						(V ** 1.5) ** 1.5 * 1.3
					)
				}
				const c = r[u],
		...
		return (
			lo(r, (l, u, c) => {
				...
				const f = t.extendToFullLines(u.seq1Range),
					p = t.getText(new T(f.start, u.seq1Range.start))
				h(p) && (m = m.deltaStart(-p.length))
				const g = t.getText(new T(u.seq1Range.endExclusive, f.endExclusive))
				h(g) && (m = m.deltaEnd(g.length))
				const d = Q.fromOffsetPairs(l ? l.getEndExclusives() : Ee.zero, c ? c.getStarts() : Ee.max),
					w = m.intersect(d)
				o.length > 0 && w.getStarts().equals(o[o.length - 1].getEndExclusives())
	...
	class si {
		...
		getBoundaryScore(e) {
			const n = e === 0 ? 0 : ii(this.lines[e - 1]),
				r = e === this.lines.length ? 0 : ii(this.lines[e])
			return 1e3 - (n + r)
		}
		getText(e) {
...
`)
	...
	class Bo {
		constructor() {
			;((this.dynamicProgrammingDiffing = new vo()), (this.myersDiffingAlgorithm = new Xs()))
		}
		computeDiff(e, n, r) {
			if (e.length <= 1 && io(e, n, (x, y) => x === y)) return new wt([], [], !1)
			if ((e.length === 1 && e[0].length === 0) || (n.length === 1 && n[0].length === 0))
				return new wt(
					[
						new Ce(new z(1, e.length + 1), new z(1, n.length + 1), [
							new he(
								new R(1, 1, e.length, e[e.length - 1].length + 1),
								new R(1, 1, n.length, n[n.length - 1].length + 1),
							),
						]),
					],
					[],
			...
			const s = r.maxComputationTimeMs === 0 ? Lt.instance : new _o(r.maxComputationTimeMs),
			...
			const u = e.map((x) => l(x.trim())),
				c = n.map((x) => l(x.trim())),
				m = new si(u, e),
				h = new si(c, n),
				f =
					m.length + h.length < 1700
						? this.dynamicProgrammingDiffing.compute(m, h, s, (x, y) =>
								e[x] === n[y] ? (n[y].length === 0 ? 0.1 : 1 + Math.log(1 + n[y].length)) : 0.99,
							)
						: this.myersDiffingAlgorithm.compute(m, h, s)
			let p = f.diffs,
				g = f.hitTimeout
			;((p = ei(m, h, p)), (p = Io(m, h, p)))
			const d = [],
				w = (x) => {
					if (i)
						for (let y = 0; y < x; y++) {
							const S = _ + y,
								V = v + y
							if (e[S] !== n[V]) {
								const U = this.refineDiff(e, n, new Q(new T(S, S + 1), new T(V, V + 1)), s, i, r)
								for (const B of U.mappings) d.push(B)
								U.hitTimeout && (g = !0)
							}
						}
				}
			let _ = 0,
				v = 0
			for (const x of p) {
				ht(() => x.seq1Range.start - _ === x.seq2Range.start - v)
				const y = x.seq1Range.start - _
				;(w(y), (_ = x.seq1Range.endExclusive), (v = x.seq2Range.endExclusive))
				const S = this.refineDiff(e, n, x, s, i, r)
				S.hitTimeout && (g = !0)
				for (const V of S.mappings) d.push(V)
			}
			w(e.length - _)
			const L = new Ut(e),
				k = new Ut(n),
			...
			let W = []
			return (
				r.computeMoves && (W = this.computeMoves(D, e, n, u, c, s, i, r)),
				ht(() => {
					function x(S, V) {
						if (S.lineNumber < 1 || S.lineNumber > V.length) return !1
						const U = V[S.lineNumber - 1]
						return !(S.column < 1 || S.column > U.length + 1)
					}
					function y(S, V) {
						return !(
							S.startLineNumber < 1 ||
							S.startLineNumber > V.length + 1 ||
							S.endLineNumberExclusive < 1 ||
							S.endLineNumberExclusive > V.length + 1
						)
					}
					for (const S of D) {
						if (!S.innerChanges) return !1
						for (const V of S.innerChanges)
							if (
								!(
									x(V.modifiedRange.getStartPosition(), n) &&
									x(V.modifiedRange.getEndPosition(), n) &&
									x(V.originalRange.getStartPosition(), e) &&
									x(V.originalRange.getEndPosition(), e)
								)
							)
								return !1
						if (!y(S.modified, n) || !y(S.original, e)) return !1
					}
					return !0
				}),
				new wt(D, W, g)
		...
		computeMoves(e, n, r, s, i, o, l, u) {
			return Co(e, n, r, s, i, o).map((h) => {
				const f = this.refineDiff(
						n,
						r,
						new Q(h.original.toOffsetRange(), h.modified.toOffsetRange()),
						o,
						l,
						u,
					),
					p = Vn(f.mappings, new Ut(n), new Ut(r), !0)
				return new Us(h, p)
			})
		}
		refineDiff(e, n, r, s, i, o) {
			const u = qo(r).toRangeMapping2(e, n),
				c = new Ot(e, u.originalRange, i),
				m = new Ot(n, u.modifiedRange, i),
				h =
					c.length + m.length < 500
						? this.dynamicProgrammingDiffing.compute(c, m, s)
						: this.myersDiffingAlgorithm.compute(c, m, s)
			let f = h.diffs
			return (
				(f = ei(c, m, f)),
				(f = ri(c, m, f, (g, d) => g.findWordContaining(d))),
				o.extendToSubwords && (f = ri(c, m, f, (g, d) => g.findSubWordContaining(d), !0)),
				(f = Fo(c, m, f)),
				(f = Vo(c, m, f)),
				{
					mappings: f.map(
						(g) => new he(c.translateRange(g.seq1Range), m.translateRange(g.seq2Range)),
					),
	...
	class $o {
		...
		get _remoteResourcesPath() {
			return ne.join(this._serverRootPath, Te.vscodeRemoteResource)
		}
		rewrite(e) {
			if (this._delegate)
				try {
			...
			const s = this._ports[n],
				i = this._connectionTokens[n]
			let o = `path=${encodeURIComponent(e.path)}`
			return (
				typeof i == 'string' && (o += `&${Uo}=${encodeURIComponent(i)}`),
				le.from({
					scheme: Pi ? this._preferredWebSchema : Te.vscodeRemoteResource,
					authority: `${r}:${s}`,
	...
	const Wo = new $o(),
		...
		St = class St {
			...
			uriToBrowserUri(e) {
				return e.scheme === Te.vscodeRemote
					? Wo.rewrite(e)
					: e.scheme === Te.file &&
						  (Mi || Ti === `${Te.vscodeFileResource}://${St.FALLBACK_AUTHORITY}`)
						? e.with({
								scheme: Te.vscodeFileResource,
								authority: e.authority || St.FALLBACK_AUTHORITY,
								query: null,
								fragment: null,
							})
						: e
			...
			toUri(e) {
				if (le.isUri(e)) return e
				if (globalThis._VSCODE_FILE_ROOT) {
					const n = globalThis._VSCODE_FILE_ROOT
					if (/^\w[\w\d+.-]*:\/\//.test(n)) return le.joinPath(le.parse(n, !0), e)
					const r = ka(n, e)
					return le.file(r)
				}
	...
	;(function (t) {
		...
		function r(i) {
			...
			typeof i == 'string'
				...
				: i instanceof URL
					? (o = i.searchParams)
					: le.isUri(i) && (o = new URL(i.toString(!0)).searchParams)
			const l = o?.get(n)
			if (l) return e.get(l)
		}
	...
	function jo(t, e) {
		...
		const r = `${t}/${e}`,
			i = `${Ho}/${r}`
		return Oo.asBrowserUri(i).toString(!0)
	}
	class Go {
		constructor(e) {
			this.replacements = e
		...
		compose(e) {
			...
			for (const l of r.replacements) {
				for (;;) {
					const h = s[0]
					if (!h || h.replaceRange.start + o + h.getNewLength() >= l.replaceRange.start) break
				...
				for (;;) {
					const h = s[0]
					if (!h || h.replaceRange.start + o > l.replaceRange.endExclusive) break
					;(c || (c = h), (m = h), s.shift(), (o += h.getNewLength() - h.replaceRange.length))
				}
				if (!c) i.push(l.delta(-o))
				else {
					const h = Math.min(c.replaceRange.start, l.replaceRange.start - u),
						f = l.replaceRange.start - (c.replaceRange.start + u)
					if (f > 0) {
						const w = c.slice(T.emptyAt(h), new T(0, f))
						i.push(w)
					}
					if (!m) throw new Y('Invariant violation: lastIntersecting is undefined')
					const p = m.replaceRange.endExclusive + o - l.replaceRange.endExclusive
					if (p > 0) {
						const w = m.slice(
							T.ofStartAndLength(m.replaceRange.endExclusive, 0),
							new T(m.getNewLength() - p, m.getNewLength()),
						)
						;(s.unshift(w), (o -= w.getNewLength() - w.replaceRange.length))
					}
					const g = new T(h, l.replaceRange.endExclusive - o),
						d = l.slice(g, new T(0, l.getNewLength()))
					i.push(d)
				}
			}
			for (;;) {
		...
		getNewRanges() {
			...
			let n = 0
			for (const r of this.replacements)
				(e.push(T.ofStartAndLength(r.replaceRange.start + n, r.getNewLength())),
					(n += r.getLengthDelta()))
			return e
		}
		isEmpty() {
	...
	class Qo extends Go {
		apply(e) {
			...
			for (const s of this.replacements)
				(n.push(e.substring(r, s.replaceRange.start)),
					n.push(s.newText),
					(r = s.replaceRange.endExclusive))
	...
	class Jo extends Xo {
		...
		removeCommonSuffixPrefix(e) {
			const n = e.substring(this.replaceRange.start, this.replaceRange.endExclusive),
				r = gn(n, this.newText),
				s = Math.min(n.length - r, this.newText.length - r, pn(n, this.newText)),
				i = new T(this.replaceRange.start + r, this.replaceRange.endExclusive - s),
				o = this.newText.substring(r, this.newText.length - s)
			return new Fe(i, o)
		}
		...
		removeCommonPrefix(e) {
			...
			return r === 0
				? this
				: this.slice(this.replaceRange.deltaStart(r), new T(r, this.newText.length))
		}
		removeCommonSuffix(e) {
			const n = this.replaceRange.substring(e),
				r = pn(n, this.newText)
			return r === 0
				? this
				: this.slice(this.replaceRange.deltaEnd(-r), new T(0, this.newText.length - r))
		}
		toJson() {
			return { txt: this.newText, pos: this.replaceRange.start, len: this.replaceRange.length }
		}
	}
	const $e = class $e extends Qo {
		static replace(e, n) {
			return new $e([new Fe(e, n)])
		}
		static compose(e) {
			if (e.length === 0) return $e.empty
			let n = e[0]
	...
	class Fe extends Jo {
		...
		tryJoinTouching(e) {
			return new Fe(this.replaceRange.joinRightTouching(e.replaceRange), this.newText + e.newText)
		}
		slice(e, n) {
			return new Fe(e, n ? n.substring(this.newText) : this.newText)
		}
	}
	fo({ StringEdit: jt, StringReplacement: Fe, TextReplacement: be, TextEdit: Wt, TextLength: Pe })
	let Hn, zn, On
	function oi() {
		return (Hn || (Hn = import(`${jo('@vscode/diff', 'dist/index.js')}`)), Hn)
	}
	function Yo(t) {
		return t
			? (On || (On = oi().then((e) => e.createDiffComputer({ useWasm: !0 }))), On)
			: (zn || (zn = oi().then((e) => e.createDiffComputer({ useWasm: !1 }))), zn)
	}
	async function li(t) {
		const e = await Yo(t)
		return new Zo(e)
...
`),
				...
				c = []
			let m = 0
			for (const p of o.edits.replacements) {
				const g = p.range.start + m,
					d = g + p.newText.length,
					w = l.getRange(new T(p.range.start, p.range.endExclusive)),
					_ = u.getRange(new T(g, d))
				;(c.push(new he(w, _)), (m += p.newText.length - (p.range.endExclusive - p.range.start)))
			}
			const h = Vn(c, s, i),
				f = []
			if (r.computeMoves)
				for (const p of o.moves) {
					const g = l.getPosition(p.range.original.start),
						d = l.getPosition(p.range.original.endExclusive),
						w = u.getPosition(p.range.modified.start),
						_ = u.getPosition(p.range.modified.endExclusive),
						v = new z(g.lineNumber, d.lineNumber),
						L = new z(w.lineNumber, _.lineNumber)
					f.push(new Us(new ce(v, L), []))
				}
			return new wt(h, f, o.hitTimeout)
		}
	}
	const Gt = {
		getLegacy: () => new wo(),
		getDefault: () => new Bo(),
		getAdvancedExternal: () => li(!1),
		getAdvancedWasm: () => li(!0),
	}
	function De(t, e) {
	...
	class b {
		constructor(e, n, r, s = 1) {
			;((this._rgbaBrand = void 0),
				...
				(this.a = De(Math.max(Math.min(1, s), 0), 3)))
		}
		static equals(e, n) {
	...
	class fe {
		constructor(e, n, r, s) {
			;((this._hslaBrand = void 0),
				...
				(this.s = De(Math.max(Math.min(1, n), 0), 3)),
				(this.l = De(Math.max(Math.min(1, r), 0), 3)),
				(this.a = De(Math.max(Math.min(1, s), 0), 3)))
		}
		static equals(e, n) {
		...
		static toRGBA(e) {
			...
			if (r === 0) o = l = u = s
				...
				const c = s < 0.5 ? s * (1 + r) : s + r - s * r,
					m = 2 * s - c
				;((o = fe._hue2rgb(m, c, n + 1 / 3)),
					(l = fe._hue2rgb(m, c, n)),
					(u = fe._hue2rgb(m, c, n - 1 / 3)))
			}
			return new b(Math.round(o * 255), Math.round(l * 255), Math.round(u * 255), i)
		}
	}
	class rt {
		constructor(e, n, r, s) {
			;((this._hsvaBrand = void 0),
				(this.h = Math.max(Math.min(360, e), 0) | 0),
				(this.s = De(Math.max(Math.min(1, n), 0), 3)),
				(this.v = De(Math.max(Math.min(1, r), 0), 3)),
				(this.a = De(Math.max(Math.min(1, s), 0), 3)))
		}
		static equals(e, n) {
				...
				l === 0
					...
					: i === n
						...
						: i === r
							? (c = (s - n) / l + 2)
							: (c = (n - r) / l + 4),
				new rt(Math.round(c * 60), u, i, e.a)
			)
		}
		static toRGBA(e) {
	...
	const O = class O {
		static fromHex(e) {
			return O.Format.CSS.parseHex(e) || O.red
		}
		static equals(e, n) {
		...
		constructor(e) {
			if (e)
				...
				else if (e instanceof fe) ((this._hsla = e), (this.rgba = fe.toRGBA(e)))
				else if (e instanceof rt) ((this._hsva = e), (this.rgba = rt.toRGBA(e)))
		...
		equals(e) {
			return (
				...
				fe.equals(this.hsla, e.hsla) &&
				rt.equals(this.hsva, e.hsva)
			)
		}
		getRelativeLuminance() {
			const e = O._relativeLuminanceForComponent(this.rgba.r),
				n = O._relativeLuminanceForComponent(this.rgba.g),
				r = O._relativeLuminanceForComponent(this.rgba.b),
		...
		lighten(e) {
			return new O(new fe(this.hsla.h, this.hsla.s, this.hsla.l + this.hsla.l * e, this.hsla.a))
		}
		darken(e) {
			return new O(new fe(this.hsla.h, this.hsla.s, this.hsla.l - this.hsla.l * e, this.hsla.a))
		}
		...
		opposite() {
			return new O(new b(255 - this.rgba.r, 255 - this.rgba.g, 255 - this.rgba.b, this.rgba.a))
		}
		mix(e, n = 0.5) {
		...
		toString() {
			return (this._toString || (this._toString = O.Format.CSS.format(this)), this._toString)
		}
		toNumber32Bit() {
			return (
				this._toNumber32Bit ||
	...
	;((O.white = new O(new b(255, 255, 255, 1))),
		(O.black = new O(new b(0, 0, 0, 1))),
		(O.red = new O(new b(255, 0, 0, 1))),
		(O.blue = new O(new b(0, 0, 255, 1))),
		(O.green = new O(new b(0, 255, 0, 1))),
		(O.cyan = new O(new b(0, 255, 255, 1))),
		(O.lightgrey = new O(new b(211, 211, 211, 1))),
		(O.transparent = new O(new b(0, 0, 0, 0))))
	let st = O
	;(function (t) {
		;(function (e) {
			;(function (n) {
				...
				function c(d, w = !1) {
					return w && d.rgba.a === 1
						...
						: `#${l(d.rgba.r)}${l(d.rgba.g)}${l(d.rgba.b)}${l(Math.round(d.rgba.a * 255))}`
				}
				n.formatHexA = c
				function m(d) {
				...
				function h(d) {
					...
					if (d.startsWith('rgba(')) {
						...
						const _ = parseInt(w.groups?.r ?? '0'),
							v = parseInt(w.groups?.g ?? '0'),
							L = parseInt(w.groups?.b ?? '0'),
							k = parseFloat(w.groups?.a ?? '0')
						return new t(new b(_, v, L, k))
					}
					if (d.startsWith('rgb(')) {
				...
				function p(d) {
					const w = d.length
					if (w === 0 || d.charCodeAt(0) !== 35) return null
					if (w === 7) {
						const _ = 16 * g(d.charCodeAt(1)) + g(d.charCodeAt(2)),
							v = 16 * g(d.charCodeAt(3)) + g(d.charCodeAt(4)),
							L = 16 * g(d.charCodeAt(5)) + g(d.charCodeAt(6))
						return new t(new b(_, v, L, 1))
					}
					if (w === 9) {
						const _ = 16 * g(d.charCodeAt(1)) + g(d.charCodeAt(2)),
							v = 16 * g(d.charCodeAt(3)) + g(d.charCodeAt(4)),
							L = 16 * g(d.charCodeAt(5)) + g(d.charCodeAt(6)),
							k = 16 * g(d.charCodeAt(7)) + g(d.charCodeAt(8))
						return new t(new b(_, v, L, k / 255))
					}
					if (w === 4) {
						const _ = g(d.charCodeAt(1)),
							v = g(d.charCodeAt(2)),
							L = g(d.charCodeAt(3))
						return new t(new b(16 * _ + _, 16 * v + v, 16 * L + L))
					}
					if (w === 5) {
						const _ = g(d.charCodeAt(1)),
							v = g(d.charCodeAt(2)),
							L = g(d.charCodeAt(3)),
							k = g(d.charCodeAt(4))
						return new t(new b(16 * _ + _, 16 * v + v, 16 * L + L, (16 * k + k) / 255))
					}
					return null
				}
				n.parseHex = p
	...
	function Ko(t, e) {
		if (!t) return
		const n = st.Format.CSS.parseHex(e)
		if (n) return { range: t, color: jn(n.rgba.r, n.rgba.g, n.rgba.b, n.rgba.a) }
	}
	function ci(t, e, n) {
		if (!t || e.length !== 1) return
		const s = e[0].values(),
			i = ui(s)
		return { range: t, color: jn(i[0], i[1], i[2], n ? i[3] : 1) }
	}
	function hi(t, e, n) {
		if (!t || e.length !== 1) return
		const s = e[0].values(),
			i = ui(s),
			o = new st(new fe(i[0], i[1] / 100, i[2] / 100, n ? i[3] : 1))
		return { range: t, color: jn(o.rgba.r, o.rgba.g, o.rgba.b, o.rgba.a) }
	}
	function vt(t, e) {
		...
		if (r.length > 0)
			for (const s of r) {
				...
				if (o === 'rgb') {
					...
					u = hi(_t(t, s), vt(l, c), !0)
				} else o === '#' && (u = Ko(_t(t, s), o + l))
				u && e.push(u)
			}
		return e
	}
	function t1(t) {
		return !t || typeof t.getValue != 'function' || typeof t.positionAt != 'function' ? [] : e1(t)
	}
	const n1 = /^-+|-+$/g,
		mi = 100,
		r1 = 5
	...
	function i1(t, e) {
		...
		for (let s = 1; s <= r; s++) {
			const i = t.getLineContent(s),
				o = i.match(e.foldingRules.markers.start)
			if (o) {
				const l = {
					startLineNumber: s,
				...
				if (l.endColumn > l.startColumn) {
					const u = { range: l, ...o1(i.substring(o[0].length)), shouldBeInComments: !1 }
	...
	function a1(t, e) {
		...
		const s = Qa(e.markSectionHeaderRegex),
			i = new RegExp(e.markSectionHeaderRegex, `gdm${s ? 's' : ''}`)
		if (ji(i)) return n
		for (let o = 1; o <= r; o += mi - r1) {
			const l = Math.min(o + mi - 1, r),
				u = []
			for (let h = o; h <= l; h++) u.push(t.getLineContent(h))
			const c = u.join(`
`)
			i.lastIndex = 0
			let m
			for (; (m = i.exec(c)) !== null; ) {
				const h = c.substring(0, m.index),
					f = (h.match(/\n/g) || []).length,
					p = o + f,
					g = m[0].split(`
`),
...
`) + 1,
					v = m.index - _ + 1,
					L = g[g.length - 1],
					k = d === 1 ? v + m[0].length : L.length + 1,
					D = { startLineNumber: p, startColumn: v, endLineNumber: w, endColumn: k },
					W = (m.groups ?? {}).label ?? '',
					x = ((m.groups ?? {}).separator ?? '') !== '',
					y = { range: D, text: W, hasSeparatorLine: x, shouldBeInComments: !0 }
	...
	function o1(t) {
		...
		return ((t = t.replace(n1, '')), { text: t, hasSeparatorLine: e })
	}
	class fi {
		get isRejected() {
	...
	const re = class re {
		...
		static tee(e) {
			...
			const s = new fi(),
				...
				o = new re(async (u) => ((n = u), i(), s.p)),
				l = new re(async (u) => ((r = u), i(), s.p))
			return [o, l]
		}
		map(e) {
	...
	class m1 {
		...
		$acceptNewModel(e) {
			this._models[e.url] = new f1(le.parse(e.url), e.lines, e.EOL, e.versionId)
		}
	...
	class f1 extends h1 {
		...
		findMatches(e) {
			...
			for (let r = 0; r < this._lines.length; r++) {
				const s = this._lines[r],
					i = this.offsetAt(new q(r + 1, 1)),
					o = s.matchAll(e)
				for (const l of o) ((l.index || l.index === 0) && (l.index = l.index + i), n.push(l))
			}
			return n
		}
		getLinesContent() {
		...
		getWordAtPosition(e, n) {
			const r = Fn(e.column, Is(n), this._lines[e.lineNumber - 1], 0)
				...
				*[Symbol.iterator]() {
					for (;;)
						if (o < l.length) {
							...
							;((o += 1), yield u)
						} else if (s < n.length) ((i = n[s]), (l = r(i, e)), (o = 0), (s += 1))
						else break
				},
			}
		}
		...
		getValueInRange(e) {
			...
			for (let o = r + 1; o < s; o++) i.push(this._lines[o])
	...
	const We = class We {
		...
		async $computeUnicodeHighlights(e, n, r) {
			...
			return s
				? ro.computeUnicodeHighlights(s, n, r)
				: {
						ranges: [],
						hasMore: !1,
		...
		async $computeDiff(e, n, r, s) {
			...
			if (!i || !o) return null
			const l = await d1(s)
			return We.computeDiff(i, o, r, l)
		}
		static computeDiff(e, n, r, s) {
			const i = e.getLinesContent(),
				o = n.getLinesContent(),
				l = s.computeDiff(i, o, r),
				u = l.changes.length > 0 ? !1 : this._modelsAreIdentical(e, n)
			function c(m) {
				return m.map((h) => [
		...
		async $computeMoreMinimalEdits(e, n, r) {
			...
			n = n.slice(0).sort((u, c) => {
				if (u.range && c.range) return R.compareRangesUsingStarts(u.range, c.range)
				const m = u.range ? 0 : 1,
					h = c.range ? 0 : 1
				return m - h
			})
			...
			for (let u = 1; u < n.length; u++)
				R.getEndPosition(n[l].range).equals(R.getStartPosition(n[u].range))
					? ((n[l].range = R.fromPositions(
							R.getStartPosition(n[l].range),
							R.getEndPosition(n[u].range),
						)),
						(n[l].text += n[u].text))
					: (l++, (n[l] = n[u]))
			n.length = l + 1
			for (let { range: u, text: c, eol: m } of n) {
				if ((typeof m == 'number' && (o = m), R.isEmpty(u) && !c)) continue
				const h = s.getValueInRange(u)
				if (((c = c.replace(/\r\n|\n|\r/g, s.eol)), h === c)) continue
				if (Math.max(c.length, h.length) > We._diffLimit) {
					i.push({ range: u, text: c })
					continue
				}
				const f = ha(h, c, r),
					p = s.offsetAt(R.lift(u).getStartPosition())
				for (const g of f) {
					const d = s.positionAt(p + g.originalStart),
						w = s.positionAt(p + g.originalStart + g.originalLength),
						_ = {
			...
			e: for (const u of e) {
				...
				if (c) {
					for (const m of c.words(o))
						if (!(m === n || !isNaN(Number(m))) && (l.add(m), l.size > We._suggestionsLimit))
							break e
				}
			}
		...
		async $navigateValueSet(e, n, r, s, i) {
			...
			const u = o.getValueInRange(n),
				c = o.getWordAtPosition({ lineNumber: n.startLineNumber, column: n.startColumn }, l)
			if (!c) return null
			const m = o.getValueInRange(c)
			return _n.INSTANCE.navigateValueSet(n, u, c, m, r)
		}
		$fmr(e, n) {
	...
	let Gn = We
	typeof importScripts == 'function' && (globalThis.monaco = za())
	function d1(t) {
		switch (t) {
			case 'legacy':
				return Gt.getLegacy()
			case 'advanced':
				return Gt.getDefault()
			case 'advanced-external':
				return Gt.getAdvancedExternal()
			case 'advanced-wasm':
				return Gt.getAdvancedWasm()
		}
	}
	ua(() => new Gn(null))
})()
