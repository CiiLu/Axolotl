...
import { consumeUpgradeFlow, parkUpgradeFlow } from '@/helpers/upgrade-return-state'
...
onMounted(() => {
	const snapshot = consumeUpgradeFlow(props.instance.id, route.fullPath)
	if (snapshot) flow.hydrate(snapshot)
})
