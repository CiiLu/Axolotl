			...
			>
				...
				>
					...
					<div class="min-w-0">
						...
						<span class="ml-2 break-words text-secondary">{{ issue.message || issue.code }}</span>
					</div>
				</div>
				...
				<div class="overflow-hidden rounded-lg border border-solid border-surface-4">
					<article
						...
						class="flex flex-col gap-3 border-0 border-b border-solid border-surface-4 bg-surface-2 p-3 last:border-b-0 sm:flex-row sm:items-start"
					>
						<div class="flex min-w-0 flex-1 items-start gap-3">
							<img
								v-if="itemIcon(group.item)"
							...
							<div class="min-w-0 flex-1">
								...
								>{{ itemName(group.item) }}</RouterLink>
								<div v-else class="truncate font-semibold text-contrast">{{ itemName(group.item) }}</div>
								...
								>
									<div
										...
										:key="issueKey(issue, index)"
										class="flex gap-2 text-sm text-red"
									>
										<XIcon class="mt-0.5 size-4 shrink-0" aria-hidden="true" />
										<span class="break-words">{{ issue.message || issueTitle(issue) }}</span>
									</div>
									<div
										v-for="(issue, index) in group.warnings"
										:key="issueKey(issue, index)"
										class="flex gap-2 text-sm text-orange"
									>
						...
						<div
							class="flex w-full min-w-0 shrink-0 flex-col items-start gap-2 sm:w-auto sm:max-w-[45%] sm:items-end"
						>
							<div
								v-if="requiresNoCompatibleResolution(group.item)"
								class="max-w-xl text-sm text-secondary"
							>
								{{ formatMessage(messages.noCompatibleExplanation) }}
							</div>
							<div
								v-if="
									requiresNoCompatibleResolution(group.item) || warningSupportsResolution(group)
								"
							...
							>
								...
								<ButtonStyled v-if="group.item.resolution.action !== 'upgrade'" type="transparent" size="small">
									<button :disabled="itemControlsDisabled(group.item)" @click="resetAction(group.item)">
										{{ formatMessage(messages.undo) }}
									</button>
								</ButtonStyled>
							</div>
							<div
								v-if="requiresPrereleaseConfirmation(group.item)"
								class="flex max-w-xl flex-col gap-2"
							>
								<span class="text-sm text-secondary">
									{{ formatMessage(messages.prereleaseExplanation) }}
									<template v-if="group.item.candidateReleaseIds[0]">
										{{
											formatMessage(messages.candidateRelease, {
												version: group.item.candidateReleaseIds[0],
											})
										}}
									</template>
								</span>
...
import { Accordion, ButtonStyled, defineMessages, formatLoaderLabel, useVIntl } from '@modrinth/ui'
...
const issueGroups = computed(() => groupUpgradeIssues(plan.value))
...
const blockingContentIdSet = computed(
	() => new Set(issueGroups.value.blocking.map((group) => group.item.contentId)),
)
...
function versionChange(item: InstanceUpgradePlanItem): string {
	const current = contentMetadata(item).currentVersion ?? formatMessage(messages.unknownVersion)
	const target =
		selectionByContentId.value.get(item.contentId)?.targetReleaseId ?? item.candidateReleaseIds[0]
	return target
		? formatMessage(messages.versionChange, { current, target })
		: formatMessage(messages.currentVersionOnly, { current })
}

function targetRelease(item: InstanceUpgradePlanItem): string | null {
	return selectionByContentId.value.get(item.contentId)?.targetReleaseId ?? item.candidateReleaseIds[0] ?? null
}

function actionLabel(item: InstanceUpgradePlanItem): string {
	if (blockingContentIdSet.value.has(item.contentId)) {
		return formatMessage(messages.needsAttention)
	}
	const action = selectionByContentId.value.get(item.contentId)?.action ?? item.resolution.action
	if (action === 'disable') return formatMessage(messages.actionDisable)
	if (action === 'keep') return formatMessage(messages.actionKeep)
	return formatMessage(messages.actionUpgrade)
}
...
function requiresPrereleaseConfirmation(item: InstanceUpgradePlanItem): boolean {
	return (
		!item.resolution.allowPrerelease &&
		(item.status === 'prerelease_only' || hasBlockingIssue(item, 'prerelease_only'))
	)
}

function dependencyVersionChange(change: InstanceUpgradeDependencyChange): string {
	const current = change.currentReleaseId ?? formatMessage(messages.unknownVersion)
	return change.targetReleaseId
		? formatMessage(messages.versionChange, { current, target: change.targetReleaseId })
		: formatMessage(messages.currentVersionOnly, { current })
}
...
function issueTitle(issue: InstanceUpgradeIssue): string {
	if (issue.code === 'prerelease_only') return formatMessage(messages.issuePrerelease)
	if (issue.code === 'no_compatible_release') return formatMessage(messages.issueNoCompatible)
	if (
	...
	if (
		['no_compatible_shader_runtime', 'shader_runtime_missing', 'shader_runtime_unknown'].includes(
			issue.code,
		)
	)
		return formatMessage(messages.issueShader)
	return formatMessage(messages.issueGeneric)
}
...
async function runBulkAction(action: 'keep' | 'disable') {
	...
	try {
		...
		const resolutions = initialContentIds.flatMap((contentId) => {
		...
		if (result.failed.length || result.skipped.length) {
			bulkError.value = formatMessage(messages.bulkPartial, {
