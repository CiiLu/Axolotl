...
impl SlowDetector {
	...
	pub fn progress(&mut self, bytes: u64, source: &str) -> Option<SlowEvent> {
		...
		if no_progress_secs >= self.preset.no_progress_secs() {
			...
			if remaining.map_or(true, |value| value >= self.preset.min_remaining_bytes()) {
				return Some(SlowEvent {
					rule: SlowRule::R1NoProgress,
					engine: "xmcl".to_string(),
					source: source.to_string(),
					detail: format!("no_progress={no_progress_secs}s elapsed={elapsed}s"),
				});
			}
		}
	...
	pub fn record_segment_waste(&mut self, wasted_bytes: u64, source: &str) -> Option<SlowEvent> {
...
mod tests {
	...
	fn no_progress_fires_after_preset_window() {
		...
		assert!(matches!(event, Some(SlowEvent { rule: SlowRule::R1NoProgress, .. })));
	}
	...
	fn route_switch_fires_after_preset_limit() {
		...
		assert!(matches!(fired, Some(SlowEvent { rule: SlowRule::R4FrequentSwitches, .. })));
	}
}
