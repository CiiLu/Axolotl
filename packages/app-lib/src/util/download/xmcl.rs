...
fn reputation_path() -> Option<std::path::PathBuf> {
	crate::State::get_if_initialized()
		.map(|state| state.directories.settings_dir.join("download-reputation.json"))
}
...
async fn load_reputation_if_needed() {
	...
	let mut reputation = REPUTATION.lock().unwrap_or_else(|error| error.into_inner());
	...
	if let Some(hosts) = value.get("hosts").and_then(serde_json::Value::as_object) {
...
async fn download_attempt(
	...
	let Some(authority) = authority_of(url) else {
		return Err(AttemptError::Http {
			error: crate::ErrorKind::NetworkError(format!("invalid url {url}")).into(),
		...
		if first_byte_at.is_some() && is_reassignable(&authority) {
			if !no_abort {
				...
				if window_start.elapsed() >= XMCL_SLOW_WINDOW {
					let window_ms = window_start.elapsed().as_millis().max(1) as u64;
...
async fn run_single_stream(
	...
	Err(last_error.unwrap_or_else(|| {
		crate::ErrorKind::NetworkError("download failed with no attempts".to_string()).into()
	}))
}
...
async fn download_segments(
	...
	for (index, segment_path) in segment_paths.iter().enumerate() {
		let start = (index as u64).saturating_mul(chunk_size);
		let end = start.saturating_add(chunk_size).min(total).saturating_sub(1);
...
async fn concatenate_segments(part_path: &Path, total: u64) -> crate::Result<()> {
		...
		if total >= SEGMENTED_DOWNLOAD_THRESHOLD && initial_offset == 0 {
			...
			match download_segments(request, routes, part_path, total, flow_started_at, &mut progress).await {
				...
				Err(SegmentError::RangeNotSupported) => {
					tracing::debug!("Range not supported, falling back to single-stream XMCL download");
