...
pub(super) fn versioned_terracotta_binary_name(version: &str, platform: &str) -> String {
...
pub(super) fn find_terracotta_executable(
	...
	while let Some(current) = pending.pop() {
		...
		for entry in entries.flatten() {
			...
			if (name == terracotta_binary_name() || name.starts_with("terracotta-"))
...
pub(super) async fn install_terracotta_binary(
	...
	if backup.exists() {
		tokio::fs::remove_file(&backup)
			.await
			.wrap_err("failed to remove terracotta backup after installation")?;
	}
	Ok(())
}

pub(super) async fn cleanup_legacy_versions(new_version: &str) -> eyre::Result<()> {
