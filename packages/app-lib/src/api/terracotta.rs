...
pub async fn download_terracotta(version: Option<String>) -> eyre::Result<()> {
	{
		let mut state = TERRACOTTA_STATE.lock().unwrap().unwrap();
	...
	let url = terracotta_download_url(&version, platform);
	...
	let response = crate::util::fetch::INSECURE_REQWEST_CLIENT
		...
		.send()
		.await
		.wrap_err_with(|| format!("failed to download terracotta from {url}"))?;
...
pub async fn get_state() -> TerracottaState {
	TERRACOTTA_STATE.lock().unwrap().unwrap().clone()
}

pub async fn get_meta() -> eyre::Result<TerracottaMeta> {
	let state = TERRACOTTA_STATE.lock().unwrap().unwrap();
	let port = state
		.http_port
...
pub async fn start_terracotta(
	...
	let mut process_guard = PROCESS.lock().unwrap().unwrap();
