...
use super::manifest::{InstallState, read_manifest, server_path, write_manifest};
...
pub async fn install_forge(
	...
	let installer_url = format!("{FORGE_MAVEN}/{mc_version}-{build}/{installer_name}");

	log(server_id, &format!("Downloading Forge installer ({installer_name})")).await?;
	download_to_dir(server_id, &dir, &installer_url, &installer_name, None).await?;
	...
	if !output.status.success() {
		...
		manifest.install_error = Some("Forge installer exited with an error".to_string());
		write_manifest(&dir, &manifest).await?;
